...
if ($secu->validadmin($admin, $kunci) == false) {
	...
	switch ($act) {
		case "input":
			...
			$kode = $data->transcode("TTR/", '/' . $data->romawi(date('m')).'/'.date('y'), "kode_ttr", "transaksi_transferretur");
			...
			if ($type == 'IN') {
				...
				if (count($msgBugs) == 0) {
					...
					$ch = curl_init();
					curl_setopt($ch, CURLOPT_URL, $sourceUrl . "/api/postProductReturTransfer.php?encrypt=" . $encrypt . "&act=" . $act);
					curl_setopt($ch, CURLOPT_POST, 1);
					curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($_POST));
					curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
					$res = curl_exec($ch);
		...
		case "approval":
			...
			fwrite($logFile, date('Y-m-d H:i:s') . " - APPROVAL REQUEST: " . json_encode($_POST) . "\n");
			...
			try {
				...
				if (!$transfer) {
					...
					array_push($msgBugs, "Status transfer retur tidak valid untuk approval");
				}
			} catch (PDOException $e) {
				array_push($msgBugs, $e->getMessage());
			...
			if (empty($msgBugs)) {
				...
				try {
					...
					// If approved, update the inventory
					if ($status === 'Approved') {
						...
						while ($item = $details->fetch(PDO::FETCH_ASSOC)) {
							...
							$updateInv->bindParam(':admin', $admin, PDO::PARAM_STR);
					...
					if (!$targetApp) {
						...
						// For sync we use the kode_ttr as it might be referenced as kode_ext_ttr in the other system
						$apiUrl = rtrim($targetApp['base_url_apl'], '/') . '/api/postProductReturTransfer.php?act=approval';
			...
			$message = $success ? "Approval transfer retur berhasil diproses" : "Gagal memproses approval";
