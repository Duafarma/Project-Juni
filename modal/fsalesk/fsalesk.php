<?php
	...
	switch($modal){
		...
		$read	= $conn->prepare("SELECT * FROM transaksi_faktur_konsinyasi WHERE id_tfk=:kode");
		$read->bindParam(':kode', $kode, PDO::PARAM_STR);
		$read->execute();
		$view	= $read->fetch(PDO::FETCH_ASSOC);
?>
        <div class="modal-header">
            <h6 class="modal-title" id="exampleModalLabel">Update Data - Faktur Konsinyasi</h6>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true"><i class="fa fa-times-circle"></i></span>
            </button>
        </div>
        <form id="formsalespnpk" action="#" method="post" autocomplete="off">
        <input type="hidden" name="namamodal" id="namamodal" value="fsalesk" readonly="readonly" />
        ...
        <div class="modal-body">
            <div class="row">
                <div class="form-group col-md-6">
                    <label>Nomor Faktur Konsinyasi<span class="tx-danger">*</span></label>
            ...
            <div class="row">
                <div class="form-group col-md-6">
                    <label>Nomor SJ Konsinyasi <span class="tx-danger">*</span></label>
...
<?php
		...
		case 'delete':
		$kode	= $secu->injection($_GET['keycode']);
		$read	= $conn->prepare("SELECT status_tfk, kode_tfk FROM transaksi_faktur_konsinyasi WHERE id_tfk=:kode");
		$read->bindParam(':kode', $kode, PDO::PARAM_STR);
		$read->execute();
		$view	= $read->fetch(PDO::FETCH_ASSOC);
?>
        ...
        <div class="modal-body">
            <div class="row">
                <div class="form-group col-md-12">
                    <label><?php echo(in_array($view['status_tfk'], array('Konsinyasi','Selesai','Sebagian')) ? 'Hapus data faktur Konsinyasi?' : 'Data sudah diproses, tidak dapat dihapus!' ); ?> <span class="tx-danger">*</span></label>
                </div>
			</div>
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary btn-xs" data-dismiss="modal">Batal</button>
            <?php echo(in_array($view['status_tfk'], array('Konsinyasi','Selesai','Sebagian')) ? '<button type="submit" id="bsave" class="btn btn-dark btn-xs">Hapus</button>' : ''); ?>
        </div>
		</form>
<?php
		break;
	...
	$conn	= $base->close();
?>
	<script type="text/javascript" src="<?php echo("$sistem/config/js/tegar.js"); ?>"></script>
