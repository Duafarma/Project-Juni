	...
	if($secu->validadmin($admin, $kunci)==false){
		...
		switch($menu){
			case "input":
				...
				$nama_p	= $secu->injection($_POST['nama_p']);
				...
				$zat_pre	= $secu->injection($_POST['zat_pre']);
				$kekuatan	= $secu->injection($_POST['kekuatan']);

				$save	= $conn->prepare("INSERT INTO produk VALUES(:kode, :kate,  :satuan, :nomor, :nama,:nama_p, :berat, :rak, :sek, :mini, :active, :obat, :kodep, :nie, :tgl, :code, :zat_pre, :kekuatan, :catat, :admin, :catat, :admin)");
				...
				$save->bindParam(":nama_p", $nama_p, PDO::PARAM_STR);
				...
				$save->bindParam(":zat_pre", $zat_pre, PDO::PARAM_STR);
				$save->bindParam(":kekuatan", $kekuatan, PDO::PARAM_STR);
				...
				*/
				...
				if($save==true){
					...
					setcookie('pesan', 'Data produk DPE berhasil diinput.', time() + 5, '/');
				} else {
					setcookie('info', 'danger', time() + 5, '/');
					setcookie('pesan', 'Data produk DPE gagal diinput.', time() + 5, '/');
				}
			break;
			case "update":
				...
				$sek	= $secu->injection($_POST['sek']);
				$mini	= str_replace('.', '', $_POST['minstok']);
				$edit	= $conn->prepare("UPDATE produk SET id_kpr=:kate, id_spr=:satuan,nama_p=:nama_p, kategori_obat=:obat, kode_produk_jadi=:kodep, no_nie=:nie, tgl_nie=:tgl, new_code=:code, zat_pre=:zat_pre, kekuatan=:kekuatan, kode_pro=:nomor, nama_pro=:nama, berat_pro=:berat, rak_pro=:rak, section_pro=:sek, minstok_pro=:mini, updated_at=:catat, updated_by=:admin WHERE id_pro=:kode");
				$edit->bindParam(":kode", $kode, PDO::PARAM_STR);
				$edit->bindParam(":kate", $kate, PDO::PARAM_STR);
				$edit->bindParam(":satuan", $satuan, PDO::PARAM_STR);
				$edit->bindParam(":nama_p", $nama_p, PDO::PARAM_STR);
				...
				$edit->bindParam(":zat_pre", $zat_pre, PDO::PARAM_STR);
				$edit->bindParam(":kekuatan", $kekuatan, PDO::PARAM_STR);
				...
				if($edit==true){
					setcookie('info', 'success', time() + 5, '/');
					setcookie('pesan', 'Data produk DPE berhasil diupdate.', time() + 5, '/');
				} else {
					setcookie('info', 'danger', time() + 5, '/');
					setcookie('pesan', 'Data produk DPE gagal diupdate.', time() + 5, '/');
				}
			break;
			case "delete":
				$kode	= $secu->injection($_POST['keycode']);
				...
				*/
				...
				if($remove==true){
					setcookie('info', 'success', time() + 5, '/');
					setcookie('pesan', 'Data produk DPE berhasil dihapus.', time() + 5, '/');
				} else {
					setcookie('info', 'danger', time() + 5, '/');
					setcookie('pesan', 'Data produk DPE gagal dihapus.', time() + 5, '/');
				}
			break;
		}
		$url	= 'produkdpe';
