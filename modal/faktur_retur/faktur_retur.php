<?php
	...
	switch($modal){
		...
		$read	= $conn->prepare("SELECT * FROM faktur_retur WHERE id_fkr=:kode");
        ...
        <div class="modal-body">
            <div class="row">
                <div class="form-group col-md-6">
                    ...
                    <input type="text" name="nomorfaktur" class="form-control" value="<?php echo($view['kode_fkr']); ?>" placeholder="Type here..." required="required" />
                </div>
                <div class="form-group col-md-6">
            ...
            <div class="row">
                <div class="form-group col-md-6">
                    ...
                    <input type="text" name="nomorsj" class="form-control" value="<?php echo($view['sj_fkr']); ?>" placeholder="Type here..." required="required" />
                </div>
                <div class="form-group col-md-6">
                    <label>Tgl. SJ <span class="tx-danger">*</span></label>
                    <input type="text" name="tglsj" class="form-control fortgl" value="<?php echo($view['tglsj_fkr']); ?>" placeholder="9999-99-99" required="required" />
                </div>
			</div>
            <div class="row">
                <div class="form-group col-md-6">
                    <label>Nomor PO <span class="tx-danger">*</span></label>
                    <input type="text" name="nomorpo" class="form-control" value="<?php echo($view['po_fkr']); ?>" placeholder="Type here..." required="required" />
                </div>
                <div class="form-group col-md-6">
                    <label>Tgl. PO <span class="tx-danger">*</span></label>
                    <input type="text" name="tglpo" class="form-control fortgl" value="<?php echo($view['tglpo_fkr']); ?>" placeholder="9999-99-99" required="required" />
                </div>
			</div>
            <div class="row">
                <div class="form-group col-md-6">
...
<?php
		...
		case 'delete':
		$kode	= $secu->injection($_GET['keycode']);
		$read	= $conn->prepare("SELECT status_fkr, kode_fkr FROM faktur_retur WHERE id_fkr=:kode");
		$read->bindParam(':kode', $kode, PDO::PARAM_STR);
		$read->execute();
		$view	= $read->fetch(PDO::FETCH_ASSOC);
?>
        ...
        <div class="modal-body">
            <div class="row">
                <div class="form-group col-md-12">
                    <label><?php echo(($view['status_fkr']==='Faktur' || $view['status_fkr']==='Tagihan') ? 'Hapus data faktur retur?' : 'Data sudah diproses, tidak dapat dihapus!' ); ?> <span class="tx-danger">*</span></label>
                </div>
			</div>
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary btn-xs" data-dismiss="modal">Batal</button>
            <?php echo(($view['status_fkr']==='Faktur' || $view['status_fkr']=='Tagihan') ? '<button type="submit" id="bsave" class="btn btn-dark btn-xs">Hapus</button>' : ''); ?>
        </div>
		</form>
<?php
		break;
