<?php
	...
	if($secu->validadmin($admin, $kunci)==false){
		...
		switch($act){
			case "input":
				$id		= '';
				$kode	    = $data->basecode('UPJ', 5, 'id_tfb', 'upload_f_pajak');	
                $tgl        = $secu->injection($_POST['tanggal']);
			
				$save	= $conn->prepare("INSERT INTO upload_f_pajak VALUES(:kode, :tgl, :catat, :admin, :catat, :admin)");
				...
				while($no<$jumlah)
				{
					...
					// Save
					$save	= $conn->prepare("INSERT INTO upload_f_pajak_detail VALUES(:id, :kode, :no_faktur, :ket, 'sudah',:catat, :admin, :catat, :admin)");
					$save->bindParam(":id", $id, PDO::PARAM_STR);
					$save->bindParam(":kode", $kode, PDO::PARAM_STR);
                    ...
                    $no_faktur			= $secu->injection(@$_POST['no_faktur'][$no]);
                    $update_tfk			= $conn->prepare("UPDATE transaksi_faktur SET upload_f_pajak='sudah' WHERE id_tfk= :no_faktur");
