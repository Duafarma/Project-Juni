	...
	if($valid==false){
		...
		switch($menu){
			case 'input':
			    ...
			    $sj_tre	= $secu->injection($_POST['sj_tre']);
				...
				if($view['proses_tor']==='Open'){
					...
					//STOK
					$save	= $conn->prepare("INSERT INTO produk_stokdetail SELECT '', id_trd, id_pro, bcode_trd,tbcode_trd, :tgltrm, jumlah_trd,0,jumlah_trd,'0',gudang,'belum so','0','active', created_at, created_by, updated_at, updated_by FROM transaksi_receivedetail WHERE id_tre=:kode");
					$save->bindParam(':kode', $kode, PDO::PARAM_STR);
					$save->bindParam(':tgltrm', $tgltrm, PDO::PARAM_STR);
					$save->execute();
					//RECEIVE ORDER
					$save	= $conn->prepare("INSERT INTO transaksi_receive VALUES(:kode, :nol, :order, :sup, :faktur,:sj_tre, :tglfak, :tgltrm, :stotal, :ppn, :pph, :gtotal, :limit, :status, :catat, :admin, :catat, :admin)");
					$save->bindParam(':kode', $kode, PDO::PARAM_STR);
					$save->bindParam(':nol', $nol, PDO::PARAM_STR);
					...
					if($save==true){
						//RIWAYAT
						$riwayat= $conn->query("INSERT INTO riwayat VALUES('', '$kode', 'Penerimaan Barang DPE', 'Create', '', '$catat', '$admin')");
						setcookie("info", "success", time() + 5, "/");
						setcookie("pesan", "Penerimaan barang DPE berhasil ditambahkan...", time() + 5, "/");
					} else {
						setcookie("info", "danger", time() + 5, "/");
						setcookie("pesan", "Penerimaan barang DPE gagal ditambahkan...", time() + 5, "/");
					}
				} else {
					setcookie("info", "danger", time() + 5, "/");
			...
			case "faktur":
				...
				//EDIT
				$edit	= $conn->prepare("UPDATE transaksi_receive SET fak_tre=:nomor, sj_tre=:sj_tre, tglfak_tre=:tglfak, tgl_limit=:tgltem, tgl_tre=:tglter, updated_at=:catat, updated_by=:admin WHERE id_tre=:kode");
				$edit->bindParam(':kode', $kode, PDO::PARAM_STR);
				$edit->bindParam(':nomor', $nomor, PDO::PARAM_STR);
				$edit->bindParam(':sj_tre', $sj_tre, PDO::PARAM_STR);
				...
				if($edit==true){
					//RIWAYAT
					$riwayat= $conn->query("INSERT INTO riwayat VALUES('', '$kode', 'Penerimaan Barang DPE', 'Update', 'Faktur $nomor', '$catat', '$admin')");
					setcookie("info", "success", time() + 5, "/");
			...
			case "inputitem":
				...
				if($save==true){
					//RIWAYAT
					$riwayat= $conn->query("INSERT INTO riwayat VALUES('', '$kode', 'Penerimaan Barang DPE', 'Update', '', '$catat', '$admin')");
					setcookie("info", "success", time() + 5, "/");
					setcookie("pesan", "Item penerimaan barang DPE berhasil ditambahkan...", time() + 5, "/");
				} else {
					setcookie("info", "danger", time() + 5, "/");
					setcookie("pesan", "Item penerimaan barang DPE gagal ditambahkan...", time() + 5, "/");
				}
				$url	= "rorderdpe/v/$uniq";
			break;
			case "editdetail":
				$uniq	= $secu->injection($_POST['keycode']);
				$kode	= base64_decode($uniq);
				...
				$url	= "rorderdpe/v/".base64_encode($view['id_tre']);
			break;
			case "delete":
				$uniq	= $secu->injection($_POST['keycode']);
				$kode	= base64_decode($uniq);
				...
				if($delete==true){
					...
					setcookie("info", "success", time() + 5, "/");
					setcookie("pesan", "Penerimaan barang DPE berhasil dihapus...", time() + 5, "/");
				} else {
					setcookie("info", "danger", time() + 5, "/");
					setcookie("pesan", "Penerimaan barang DPE gagal dihapus...", time() + 5, "/");
				}
				$url	= 'rorderdpe';
			break;
		}
