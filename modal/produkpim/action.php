	...
	if($secu->validadmin($admin, $kunci)==false){
		...
		switch($menu){
			...
			case "update":
				...
				if($edit==true){
					setcookie('info', 'success', time() + 5, '/');
					setcookie('pesan', 'Data produk PIM berhasil diupdate.', time() + 5, '/');
				} else {
					setcookie('info', 'danger', time() + 5, '/');
					setcookie('pesan', 'Data produk PIM gagal diupdate.', time() + 5, '/');
				}
			break;
			case "delete":
				$kode	= $secu->injection($_POST['keycode']);
				...
				*/
				...
				if($remove==true){
					setcookie('info', 'success', time() + 5, '/');
					setcookie('pesan', 'Data produk PIM berhasil dihapus.', time() + 5, '/');
				} else {
					setcookie('info', 'danger', time() + 5, '/');
					setcookie('pesan', 'Data produk PIM gagal dihapus.', time() + 5, '/');
				}
			break;
		}
		$url	= 'produkpim';
