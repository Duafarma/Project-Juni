<?php
	...
	$notin 	= empty($cart) ? "A.id_i_r!=''" : "A.id_i_r NOT IN('".str_replace("-", "', '", $cart)."')";
?>
        ...
        <div class="row">
			...
			<table id="example1" class="tabelgetdata">
				...
				<?php
					...
					LEFT JOIN produk_diskonretur AS F ON B.id_pro=F.id_pro
					WHERE $notin AND A.sisa>0 AND F.id_sup=:mitra AND C.status_phg=:status
					GROUP BY A.id_i_r, A.no_bcode, A.created_at, A.gudang, A.ed, A.sisa, B.id_pro, B.kode_pro, B.nama_pro, B.berat_pro, C.harga_phg, D.nama_kpr, D.satuan_kpr, E.nama_spr, F.persen_pds
					ORDER BY B.nama_pro, A.ed, A.created_at ASC
					");
				...
				?>
                	<tr onClick="<?php echo("getproductsalesretur('$nomor', '$hasil[id_i_r]', '$hasil[id_pro]', '$hasil[nama_pro]', '$hasil[kode_pro]', '$hasil[harga_phg]', '$hasil[berat_pro]', '$hasil[nama_kpr]', '$hasil[satuan_kpr]', '$hasil[nama_spr]', '$hasil[no_bcode]', '$hasil[ed]', '$hasil[gudang]', '$hasil[sisa]', '$hasil[persen_pds]')"); ?>">
                    	...
                    	<td><div align="right"><?php echo($hasil['sisa']); ?></div></td>
