<?php
	...
	if($secu->validadmin($admin, $kunci)==false){
		...
		switch($act){
			case "input":
				$id		= '';
				$kode	    = $data->basecode('SRHP', 5, 'id_db', 'serah_terima_faktur_pemberkasan');	
                $tgl        = $secu->injection($_POST['tanggal']);
			
				$save	= $conn->prepare("INSERT INTO serah_terima_faktur_pemberkasan VALUES(:kode, :tgl, :catat, :admin, :catat, :admin)");
				...
				while($no<$jumlah)
				{
					...
					// Save
					$save	= $conn->prepare("INSERT INTO serah_terima_faktur_pemberkasan_detail VALUES(:id, :kode, :no_faktur, :ket, 'sudah diterima',:catat, :admin, :catat, :admin)");
					$save->bindParam(":id", $id, PDO::PARAM_STR);
					$save->bindParam(":kode", $kode, PDO::PARAM_STR);
                    ...
                    $no_faktur			= $secu->injection(@$_POST['no_faktur'][$no]);
                    $update_tfk			= $conn->prepare("UPDATE transaksi_faktur SET serah_terima_pemberkasan='sudah diterima' WHERE id_tfk= :no_faktur");
				...
				//RIWAYAT
				$riwayat= $conn->query("INSERT INTO riwayat VALUES('', '$kode', 'finance Serah terima Pemberkasan - bagian Pemberkasan', 'Create', 'Serah Terima Faailing', '$catat', '$admin')");
