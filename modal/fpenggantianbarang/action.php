	...
	if($secu->validadmin($admin, $kunci)==false){
		...
		switch($menu){
			case 'faktur':
                ...
                $no_tfk_penjualan	= $secu->injection($_POST['no_tfk_penjualan']);
				...
				try {
					$qRead = "SELECT * 
								FROM transaksi_faktur_penggantian_barang
								WHERE
									po_tfk=:po_tfk OR
									kode_tfkk=:kode_tfkk
								LIMIT 1";
					...
					$read->bindParam(':kode_tfkk', $nofak, PDO::PARAM_STR);
					$read->execute();
					$dRead  = $read->fetch(PDO::FETCH_ASSOC);
                    if (is_array($dRead)) {
						array_push($msgBugs, "Nomor pengeluaran  ".$nomorpo." atau Nomor Penerimaan sudah pernah digunakan!");
					}
				...
				if (empty($msgBugs)) {
					...
					if (empty($msgBugs)) {
						$qSave = "INSERT
									INTO
										transaksi_faktur_penggantian_barang (
											id_tfk,
											id_tsl,
											id_out,
											sj_tfk,
						...
						try {
                            ...
                            $save->bindParam(':no_tfk_penjualan', $no_tfk_penjualan, PDO::PARAM_STR);
						...
						if (empty($msgBugs)) {
							$url	= "itemfpenggantianbarang/$uniq";
						}
					}
				}
			break;
			case 'items':
				...
				while($no<$jum){
					if (empty($msgBugs)) {
						...
						//INPUT
						$qSave = "INSERT 
									INTO transaksi_faktur_penggantian_barang_detail
									VALUES (:id, 
										:code, 
										:kodestok, 
										:pro, 
			...
			case 'update':
				...
				try {
					$edit	= $conn->prepare("UPDATE transaksi_faktur_penggantian_barang SET kode_tfkk=:nofak, tgl_tfk=:tglfak, sj_tfk=:nosj, tglsj_tfk=:tglsj, po_tfk=:nopo, updated_at=:catat, updated_by=:admin WHERE id_tfk=:kode");
			...
			case 'delete':
				...
				try {
					$master	= $conn->prepare("SELECT id_psd, jumlah_tfd FROM transaksi_faktur_penggantian_barang_detail WHERE id_tfk=:kode");
				...
				*/
				if (empty($msgBugs)) {
					try {
						$remove	= $conn->prepare("DELETE A, B, C FROM transaksi_faktur_penggantian_barang AS A LEFT JOIN transaksi_faktur_penggantian_barang_detail AS B ON A.id_tfk=B.id_tfk LEFT JOIN pembayaran_faktur AS C ON A.id_tfk=C.id_tfk WHERE A.id_tfk=:kode");
