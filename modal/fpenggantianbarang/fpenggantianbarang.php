<?php
	...
	switch($modal){
		...
		$read	= $conn->prepare("SELECT * FROM transaksi_faktur_penggantian_barang WHERE id_tfk=:kode");
        ...
        <div class="modal-body">
            <div class="row">
                <div class="form-group col-md-6">
                    ...
                    <input type="text" name="nomorfaktur" class="form-control" value="<?php echo($view['kode_tfkk']); ?>" placeholder="Type here..." required="required" />
                </div>
                <div class="form-group col-md-6">
...
<?php
		...
		case 'delete':
		$kode	= $secu->injection($_GET['keycode']);
		$read	= $conn->prepare("SELECT status_tfk, kode_tfkk FROM transaksi_faktur_penggantian_barang WHERE id_tfk=:kode");
		$read->bindParam(':kode', $kode, PDO::PARAM_STR);
		$read->execute();
		$view	= $read->fetch(PDO::FETCH_ASSOC);
?>
