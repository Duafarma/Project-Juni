...
$view  = $conn->prepare("SELECT id_pp, nama_program, jenis_program, min_qty, diskon_persen FROM master_program_produk WHERE id_pp=:id");
$view->bindParam(':id', $id, PDO::PARAM_INT);
$view->execute();
$prog  = $view->fetch(PDO::FETCH_ASSOC);
switch($modal){
    case "detail":
        $detail= $conn->prepare("SELECT B.id_pro, C.kode_pro, C.nama_pro, C.berat_pro, D.nama_kpr, E.nama_spr, B.harga_program FROM master_program_produk_detail AS B LEFT JOIN produk AS C ON B.id_pro=C.id_pro LEFT JOIN kategori_produk AS D ON C.id_kpr=D.id_kpr LEFT JOIN satuan_produk AS E ON C.id_spr=E.id_spr WHERE B.id_pp=:id ORDER BY C.nama_pro ASC");
        ...
        <div class="modal-body">
            <div class="mb-3">
                ...
                <div><strong>Outlets:</strong>
                    <?php
                        $outletQuery = $conn->prepare("SELECT B.nama_out FROM master_program_produk_outlet AS A LEFT JOIN outlet AS B ON A.id_out=B.id_out WHERE A.id_pp=:id ORDER BY B.nama_out ASC");
...
<?php
    break;
    case "delete":
        $jml = $conn->query("SELECT COUNT(id_ppd) AS total FROM master_program_produk_detail WHERE id_pp=$id")->fetch(PDO::FETCH_ASSOC);
?>
        ...
        <input type="hidden" name="nmenu" id="nmenu" value="master_program_produk" readonly="readonly" />
        ...
        <div class="modal-body">
            <div class="alert alert-danger">
                <strong>Konfirmasi!</strong> Hapus program <b><?php echo($prog['nama_program']); ?></b>?<br/>
        ...
        (function(){
            ...
            $(document).on('submit', '#formtransaksi.moduleDelete', function(e){
                ...
                if(nmenu !== 'master_program_produk') return;
                var $btn = $('#bsave');
                $btn.prop('disabled', true);
                if($('#imgloading').length){ $('#imgloading').html('<img src="'+usuper+'/berkas/gif/tunggu.gif" style="width:15%;"  />'); }
                $.ajax({
                    url: usuper+"/modal/master_program_produk/action.php?act=delete",
                    type: 'POST',
                    dataType: 'text',
                    ...
                    success: function(data){
                        if(String(data).trim() === 'success'){
                            ...
                            setTimeout(function(){ window.location.href = usuper+"/master_program_produk"; }, 2000);
                        } else {
