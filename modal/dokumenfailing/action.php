	...
	if($secu->validadmin($admin, $kunci)==false){
		...
		switch($act){
			case "input":
				...
				$save	= $conn->prepare("INSERT INTO dokumen_failing VALUES(:kode, :tgl, :catat, :admin, :catat, :admin)");
				...
				while($no<$jumlah)
				{
					...
					// Save only the id_tfk to dokumen_failing_detail
					$save	= $conn->prepare("INSERT INTO dokumen_failing_detail VALUES(:id, :kode, :no_faktur, :ket, 'sudah failing',:catat, :admin, :catat, :admin)");
					$save->bindParam(":id", $id, PDO::PARAM_STR);
					$save->bindParam(":kode", $kode, PDO::PARAM_STR);
                    ...
                    if($sumber == 'Cendo') {
                        $update_tfk = $conn->prepare("UPDATE transaksi_faktur SET status_failing='sudah failing' WHERE id_tfk= :no_faktur");
                        $update_tfk->bindParam(':no_faktur', $id_tfk, PDO::PARAM_STR);
                        $update_tfk->execute();
                    } else if($sumber == 'PIM') {
                        $update_tfk = $conn->prepare("UPDATE transaksi_faktur_pim SET status_failing='sudah failing' WHERE id_tfk= :no_faktur");
                        $update_tfk->bindParam(':no_faktur', $id_tfk, PDO::PARAM_STR);
                        $update_tfk->execute();
                    }
				...
				//RIWAYAT
				$riwayat= $conn->query("INSERT INTO riwayat VALUES('', '$kode', 'finance Dokumen Failing', 'Create', 'Input Tuker Faktur', '$catat', '$admin')");
