...
<?php
		...
		$kode	= $secu->injection($_GET['keycode']);
		$read	= $conn->prepare("SELECT A.id_psd, A.no_bcode, A.tgl_expired, A.gudang,A.status, A.tgl_psd, A.qty_so, A.sisa_psd,A.status, B.nama_pro, B.berat_pro,B.minstok_pro, C.harga_phg, C.hargap_phg, E.nama_spr FROM produk_stokdetail AS A LEFT JOIN produk AS B ON A.id_pro=B.id_pro LEFT JOIN produk_harga AS C ON B.id_pro=C.id_pro LEFT JOIN kategori_produk AS D ON B.id_kpr=D.id_kpr LEFT JOIN satuan_produk AS E ON B.id_spr=E.id_spr WHERE A.id_psd=:kode");
		$read->bindParam(':kode', $kode, PDO::PARAM_STR);
		$read->execute();
		$view	= $read->fetch(PDO::FETCH_ASSOC);
?>		
...
<div class="modal-header">
            <h6 class="modal-title" id="exampleModalLabel">Data Hitung Fisik</h6>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true"><i class="fa fa-times-circle"></i></span>
            </button>
        </div>
        <form id="formtransaksi" action="#" method="post" autocomplete="off">
            <input type="hidden" name="namamodal" id="namamodal" value="stockopnamein" readonly="readonly" />
        ...
        <div class="modal-body">
            <div class="row">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered">
						...
						?>
                        	<tr>
                                ...
                                <td><center><?php echo($views['awal']); ?></center></td>
                    ...
                    <td>
                             <label>Awal <span class="tx-danger">*</span></label>
							<input type="number" name="awal" id="awal" class="form-control"/>
					</td>
                    </div>
