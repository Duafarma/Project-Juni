<?php
// Penjualan RHEA Principle API - filter by MP0000000004
if ($_SERVER['REQUEST_METHOD'] !== 'GET') {
    ...
    // Validate encrypt using local self_apl key
    ...
    if (md5($tgl . '#' . $sourceKey) == $encrypt) {
        ...
        $tgl_mulai = $secu->injection(@$_GET['tgl_mulai']);
        $tgl_selesai = $secu->injection(@$_GET['tgl_selesai']);
        
        // Normalize date to YYYY-MM-DD if UI sends dd/mm/YYYY
        ...
        $tgl_mulai = $normalizeDate($tgl_mulai);
        $tgl_selesai = $normalizeDate($tgl_selesai);
        $cabang = $secu->injection(@$_GET['cabang']) ?: 'ALL';

        // Build query: Sales RHEA detail (filtered by principle MP0000000004)
        $nama_p = 'MP0000000004'; // RHEA principle
        ...
        $q = "SELECT
              ...
              WHERE C.nama_p = :nama_p";

        // Add filters
        $params = [':nama_p' => $nama_p];
        ...
        if (!empty($tgl_mulai) && !empty($tgl_selesai)) {
            $q .= " AND DATE(A.tgl_tfk) BETWEEN :tgl_mulai AND :tgl_selesai";
            $params[':tgl_mulai'] = $tgl_mulai;
            $params[':tgl_selesai'] = $tgl_selesai;
        } elseif (!empty($tgl_mulai)) {
            $q .= " AND DATE(A.tgl_tfk) >= :tgl_mulai";
            $params[':tgl_mulai'] = $tgl_mulai;
        } elseif (!empty($tgl_selesai)) {
            $q .= " AND DATE(A.tgl_tfk) <= :tgl_selesai";
            $params[':tgl_selesai'] = $tgl_selesai;
        }
