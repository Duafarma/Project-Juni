<?php
require_once('../config/connection/connection.php');
require_once('../config/connection/security.php');
require_once('../config/function/data.php');

header('Access-Control-Allow-Origin: *');
header("Content-type: application/json; charset=utf-8");
ini_set('display_errors', 0);

$secu = new Security;
$base = new DB;
$data = new Data;
$conn = $base->open();

$request  = ($_SERVER['REQUEST_METHOD'] === 'POST') ? $_POST : $_GET;
$tgl      = date('Y-m-d');
$currentYear  = (int)date('Y');
$currentMonth = (int)date('n');
$cari     = $secu->injection(@$request['caridata']);
$page     = (int)($secu->injection(@$request['halaman']) ?: 1);
$maxi     = (int)($secu->injection(@$request['maximal']) ?: 15);
$encrypt  = $secu->injection(@$request['encrypt']);
$idAplReq = $secu->injection(@$request['id_apl']);
$paginate = isset($request['paginate']) ? (int)$secu->injection(@$request['paginate']) : 1;
$mulai    = ($page > 1) ? (($page * $maxi) - $maxi) : 0;

$source    = $data->self_apl();
$sourceKey = $source['key_apl'] ?? '';
$id_apl    = $source['id_apl'] ?? '';
$nama_apl  = $source['nama_apl'] ?? '';

// ─── Helpers ────────────────────────────────────────────────────────────────

function respondStockLogJson($payload, $httpCode = 200) {
    http_response_code($httpCode);
    echo json_encode($payload);
}

function isValidStockLogEncrypt($encrypt, $tgl, $sourceKey, $authSourceKey) {
    $candidates = array();
    if ($sourceKey !== '') {
        $candidates[] = md5($tgl . '#' . $sourceKey);
    }
    if ($authSourceKey !== '' && $authSourceKey !== $sourceKey) {
        $candidates[] = md5($tgl . '#' . $authSourceKey);
    }
    return in_array($encrypt, $candidates, true);
}

function resolveStockLogAuthSource($conn, $source, $requestedApl) {
    $requestedApl = trim((string)$requestedApl);
    if ($requestedApl === '' || strtolower($requestedApl) === 'all' || $requestedApl === (string)($source['id_apl'] ?? '')) {
        return $source;
    }
    $stmt = $conn->prepare('SELECT id_apl, nama_apl, key_apl, base_url_apl FROM aplikasi WHERE id_apl = :id_apl AND active_apl = 1 LIMIT 1');
    $stmt->bindValue(':id_apl', $requestedApl, PDO::PARAM_STR);
    $stmt->execute();
    $apl = $stmt->fetch(PDO::FETCH_ASSOC);
    return $apl ?: $source;
}

function filterStockLogRowsByPeriod($rows, $year, $month) {
    $filtered = array();

    foreach ((array)$rows as $row) {
        $createdAt = trim((string)($row['created_at'] ?? ''));
        if ($createdAt === '') {
            continue;
        }

        $timestamp = strtotime($createdAt);
        if ($timestamp === false) {
            continue;
        }

        if ((int)date('Y', $timestamp) !== (int)$year || (int)date('n', $timestamp) !== (int)$month) {
            continue;
        }

        $filtered[] = $row;
    }

    return $filtered;
}

function fetchLocalStockLog($conn, $sourceInfo, $cari, $year, $month, $offset = null, $limit = null) {
    $cariLike = '%' . $cari . '%';

    $qCount = "SELECT COUNT(A.id) AS total
               FROM stock AS A
               WHERE A.nama_pro LIKE :cari
                 AND YEAR(A.created_at) = :tahun
                 AND MONTH(A.created_at) = :bulan";

    $stmtCount = $conn->prepare($qCount);
    $stmtCount->bindValue(':cari', $cariLike, PDO::PARAM_STR);
    $stmtCount->bindValue(':tahun', (int)$year, PDO::PARAM_INT);
    $stmtCount->bindValue(':bulan', (int)$month, PDO::PARAM_INT);
    $stmtCount->execute();
    $total = (int)($stmtCount->fetchColumn() ?? 0);

    $qMaster = "SELECT
                    :id_apl         AS id_apl,
                    :nama_apl       AS nama_apl,
                    A.id,
                    A.id_psd,
                    A.id_pro,
                    A.nama_pro,
                    A.nama_inventory,
                    A.no_bcode,
                    A.qty,
                    A.bcode_so,
                    A.qty_so,
                    A.selisih,
                    A.created_at,
                    A.created_by,
                    COALESCE(B.nama_adm, A.created_by) AS nama_oleh,
                    A.updated_at,
                    A.updated_by
                FROM stock AS A
                LEFT JOIN adminz AS B ON B.id_adm = A.created_by
                WHERE A.nama_pro LIKE :cari
                                    AND YEAR(A.created_at) = :tahun
                                    AND MONTH(A.created_at) = :bulan
                ORDER BY A.id DESC";

    if ($offset !== null && $limit !== null) {
        $qMaster .= ' LIMIT :mulai, :maxi';
    }

    $stmt = $conn->prepare($qMaster);
    $stmt->bindValue(':id_apl',   $sourceInfo['id_apl'],   PDO::PARAM_STR);
    $stmt->bindValue(':nama_apl', $sourceInfo['nama_apl'], PDO::PARAM_STR);
    $stmt->bindValue(':cari',     $cariLike,               PDO::PARAM_STR);
    $stmt->bindValue(':tahun',    (int)$year,              PDO::PARAM_INT);
    $stmt->bindValue(':bulan',    (int)$month,             PDO::PARAM_INT);
    if ($offset !== null && $limit !== null) {
        $stmt->bindValue(':mulai', (int)$offset, PDO::PARAM_INT);
        $stmt->bindValue(':maxi',  (int)$limit,  PDO::PARAM_INT);
    }
    $stmt->execute();

    return array(
        'total' => $total,
        'data'  => $stmt->fetchAll(PDO::FETCH_ASSOC)
    );
}

function resolveStockLogBranchBaseUrl($branch) {
    $baseUrl = trim($branch['base_url_apl'] ?? '');
    if ($baseUrl !== '') {
        return rtrim($baseUrl, '/');
    }
    $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $host   = $_SERVER['HTTP_HOST'] ?? 'localhost';
    return $scheme . '://' . $host . '/monitoring';
}

function callStockLogBranchApi($branch, $payload) {
    $apiUrl = resolveStockLogBranchBaseUrl($branch) . '/api/getStockLog.php?' . http_build_query($payload);

    if (function_exists('curl_init')) {
        $ch = curl_init($apiUrl);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_TIMEOUT, 20);
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 8);
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
        $response = curl_exec($ch);
        $httpCode = (int)curl_getinfo($ch, CURLINFO_HTTP_CODE);
        $error    = curl_error($ch);
        curl_close($ch);

        if ($response === false || $error !== '') {
            return array('ok' => false, 'message' => $error ?: 'Curl request gagal', 'http_code' => $httpCode);
        }

        $json = json_decode($response, true);
        if (!is_array($json)) {
            return array('ok' => false, 'message' => 'Response bukan JSON valid', 'http_code' => $httpCode);
        }

        return array('ok' => ($httpCode >= 200 && $httpCode < 300), 'http_code' => $httpCode, 'json' => $json);
    }

    // Fallback: file_get_contents
    $options = array(
        'http' => array(
            'method'        => 'GET',
            'timeout'       => 20,
            'ignore_errors' => true,
            'header'        => "Accept: application/json\r\n"
        )
    );
    $context  = stream_context_create($options);
    $response = @file_get_contents($apiUrl, false, $context);
    if ($response === false) {
        return array('ok' => false, 'message' => 'HTTP request gagal');
    }

    $httpCode = 200;
    if (!empty($http_response_header) && preg_match('/\s(\d{3})\s/', $http_response_header[0], $match)) {
        $httpCode = (int)$match[1];
    }

    $json = json_decode($response, true);
    if (!is_array($json)) {
        return array('ok' => false, 'message' => 'Response bukan JSON valid', 'http_code' => $httpCode);
    }

    return array('ok' => ($httpCode >= 200 && $httpCode < 300), 'http_code' => $httpCode, 'json' => $json);
}

// ─── Validate encrypt ────────────────────────────────────────────────────────

$authSource    = resolveStockLogAuthSource($conn, $source, $idAplReq);
$authSourceKey = $authSource['key_apl'] ?? $sourceKey;

if (!isValidStockLogEncrypt($encrypt, $tgl, $sourceKey, $authSourceKey)) {
    respondStockLogJson(array(
        'status'  => 'error',
        'message' => 'Unauthorized: encrypt tidak valid',
        'id_apl'  => $id_apl
    ), 401);
    $conn = $base->close();
    exit;
}

// ─── Dispatch ────────────────────────────────────────────────────────────────

try {
    $requestedApl = trim($idAplReq);

    // ── SINGLE BRANCH READ ───────────────────────────────────────────────────
    if ($requestedApl !== '' && strtolower($requestedApl) !== 'all' && $requestedApl !== $id_apl) {
        $stmtBranch = $conn->prepare('SELECT id_apl, nama_apl, base_url_apl, key_apl FROM aplikasi WHERE id_apl = :id_apl AND active_apl = 1 LIMIT 1');
        $stmtBranch->bindValue(':id_apl', $requestedApl, PDO::PARAM_STR);
        $stmtBranch->execute();
        $branch = $stmtBranch->fetch(PDO::FETCH_ASSOC);

        if (!$branch) {
            respondStockLogJson(array('status' => 'error', 'message' => 'Cabang tidak ditemukan'), 404);
            $conn = $base->close();
            exit;
        }

        $remoteResponse = callStockLogBranchApi($branch, array(
            'encrypt'  => md5($tgl . '#' . $branch['key_apl']),
            'id_apl'   => $branch['id_apl'],
            'caridata' => $cari,
            'halaman'  => $page,
            'maximal'  => $maxi,
            'paginate' => $paginate
        ));

        if (!$remoteResponse['ok']) {
            respondStockLogJson(array(
                'status'  => 'error',
                'message' => $remoteResponse['json']['message'] ?? $remoteResponse['message'] ?? 'Gagal mengambil data cabang'
            ), 502);
            $conn = $base->close();
            exit;
        }

        if (isset($remoteResponse['json']['data']) && is_array($remoteResponse['json']['data'])) {
            $remoteResponse['json']['data'] = filterStockLogRowsByPeriod($remoteResponse['json']['data'], $currentYear, $currentMonth);
            $remoteResponse['json']['total'] = count($remoteResponse['json']['data']);
        }

        respondStockLogJson($remoteResponse['json'], $remoteResponse['http_code'] ?? 200);
        $conn = $base->close();
        exit;
    }

    // ── ALL BRANCHES ─────────────────────────────────────────────────────────
    if (strtolower($requestedApl) === 'all') {
        $stmtAplikasi = $conn->query('SELECT id_apl, nama_apl, self_apl, base_url_apl, key_apl FROM aplikasi WHERE active_apl = 1 ORDER BY nama_apl ASC');
        $aplikasiList = $stmtAplikasi ? $stmtAplikasi->fetchAll(PDO::FETCH_ASSOC) : array();

        $mergedRows = array();
        $errors     = array();

        foreach ($aplikasiList as $aplInfo) {
            if ((string)$aplInfo['id_apl'] === (string)$id_apl || (int)$aplInfo['self_apl'] === 1) {
                $localResult = fetchLocalStockLog($conn, array('id_apl' => $id_apl, 'nama_apl' => $nama_apl), $cari, $currentYear, $currentMonth, null, null);
                foreach ($localResult['data'] as $row) {
                    $mergedRows[] = $row;
                }
                continue;
            }

            $remoteResponse = callStockLogBranchApi($aplInfo, array(
                'encrypt'  => md5($tgl . '#' . $aplInfo['key_apl']),
                'id_apl'   => $aplInfo['id_apl'],
                'caridata' => $cari,
                'halaman'  => 1,
                'maximal'  => $maxi,
                'paginate' => 0
            ));

            if (!$remoteResponse['ok'] || ($remoteResponse['json']['status'] ?? '') !== 'success') {
                $errors[] = array(
                    'id_apl'   => $aplInfo['id_apl'],
                    'nama_apl' => $aplInfo['nama_apl'],
                    'message'  => $remoteResponse['json']['message'] ?? $remoteResponse['message'] ?? 'Gagal'
                );
                continue;
            }

            foreach (filterStockLogRowsByPeriod($remoteResponse['json']['data'] ?? array(), $currentYear, $currentMonth) as $row) {
                $mergedRows[] = $row;
            }
        }

        // Urutkan berdasarkan id DESC (terbaru dulu) lalu nama_pro
        usort($mergedRows, function ($a, $b) {
            $cmpId = (int)($b['id'] ?? 0) - (int)($a['id'] ?? 0);
            if ($cmpId !== 0) return $cmpId;
            return strcmp((string)($a['nama_pro'] ?? ''), (string)($b['nama_pro'] ?? ''));
        });

        $total = count($mergedRows);
        $rows  = ($paginate === 0) ? $mergedRows : array_slice($mergedRows, $mulai, $maxi);

        respondStockLogJson(array(
            'status'   => 'success',
            'id_apl'   => 'all',
            'nama_apl' => 'Semua Cabang',
            'total'    => $total,
            'halaman'  => $page,
            'maximal'  => $maxi,
            'data'     => array_values($rows),
            'errors'   => $errors
        ));
        $conn = $base->close();
        exit;
    }

    // ── LOCAL READ ───────────────────────────────────────────────────────────
    $localResult = fetchLocalStockLog(
        $conn,
        array('id_apl' => $id_apl, 'nama_apl' => $nama_apl),
        $cari,
        $currentYear,
        $currentMonth,
        ($paginate === 0 ? null : $mulai),
        ($paginate === 0 ? null : $maxi)
    );

    respondStockLogJson(array(
        'status'   => 'success',
        'id_apl'   => $id_apl,
        'nama_apl' => $nama_apl,
        'total'    => $localResult['total'],
        'halaman'  => $page,
        'maximal'  => $maxi,
        'data'     => $localResult['data']
    ));

} catch (PDOException $e) {
    respondStockLogJson(array(
        'status'  => 'error',
        'message' => $e->getMessage()
    ), 500);
}

$conn = $base->close();
?>
