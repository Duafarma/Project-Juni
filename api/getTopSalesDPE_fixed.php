<?php
    ...
    // Add condition for DPE products (nama_p = MP0000000002) and VISION BLU products
    $where_conditions[] = "(C.nama_p = 'MP0000000002' OR C.nama_pro LIKE '%VISION BLU%')";

    $where_clause = !empty($where_conditions) ? 'WHERE ' . implode(' AND ', $where_conditions) : 'WHERE (C.nama_p = "MP0000000002" OR C.nama_pro LIKE "%VISION BLU%")';

    try {
        // Query untuk top sales berdasarkan total quantity per outlet+product
