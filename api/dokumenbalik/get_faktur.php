...
$status = 'belum balik';
$where = "WHERE A.status_dokumen = :status AND YEAR(A.tgl_tfk) IN (2025, 2026)";
if (!empty($search)) {
...
if (!$is_remote) {
    ...
    while ($rem = $stmt_rem->fetch()) {
        ...
        $remoteUrl = rtrim($rem['base_url_apl'], '/') . '/api/dokumenbalik/get_faktur.php?' . http_build_query([
            'is_remote' => 'true',
            'q' => $search,
            'page' => $page
        ]);
...
// Karena data remote digabung, kita asumsikan "more" jika lokal masih ada data.
// Pagination untuk remote mungkin terpotong jika timeout, ini adalah kompromi kecepatan vs akurasi
echo json_encode([
    "results" => $results,
    "pagination" => [
