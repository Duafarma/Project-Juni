...
/**
 * API Dokumen Failing
 * Lokasi: api/dokumenfailing/create.php
 */
...
$log_file = "../../logs/api_dokumenfailing_" . date('Y-m-d') . ".log";
...
try {
    ...
    $ext_id_df = $_POST['ext_id_df'] ?? null; // Ambil ext_id_df jika dari remote
    ...
    // Generate ID Lokal
    $kode = $data->basecode('TANDA', 5, 'id_df', 'dokumen_failing');
    
    // 3. Simpan Master menggunakan format INSERT yang eksplisit kolomnya
    $stmt = $conn->prepare("INSERT INTO dokumen_failing (id_df, ext_id_df, tanggal, created_at, cretaed_by, updated_at, updated_by) VALUES (:id, :ext_id, :tgl, :catat, :admin, :catat, :admin)");
    $stmt->execute([
        ':id' => $kode, 
        ':ext_id' => $ext_id_df, // Masukkan ext_id_df (berisi ID pusat jika ini di cabang)
        ':tgl' => $tanggal, 
        ':catat' => $catat, 
        ':admin' => $admin
    ]);
    ...
    foreach ($items as $item) {
        ...
        $stmt_det = $conn->prepare("INSERT INTO dokumen_failing_detail (id_df, no_faktur, kode_tfk_ext, nama_out_ext, tgl_faktur_ext, ket, status_failing, created_at, created_by, updated_at, updated_by) VALUES (:id, :no_faktur, :kode_ext, :nama_ext, :tgl_ext, :ket, 'sudah failing', :catat, :admin, :catat, :admin)");
        $stmt_det->execute([
            ':id'       => $kode, 
            ':no_faktur' => $id_tfk, 
            ':kode_ext'  => $kode_tfk,
        ...
        $conn->prepare("UPDATE $tbl SET status_failing = 'sudah failing' WHERE kode_tfk = ?")->execute([$kode_tfk]);
    }
    ...
    if (!$is_remote) {
        ...
        while ($rem = $stmt_rem->fetch()) {
            ...
            $ch = curl_init(rtrim($rem['base_url_apl'], '/') . '/api/dokumenfailing/create.php');
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query([
                'is_remote' => 'true', 
                'ext_id_df' => $kode, // Kirim ID lokal ini sebagai ext_id_df untuk cabang
                'admin'     => $admin,
                'tanggal'   => $tanggal, 
                'items'     => $items
            ]));
