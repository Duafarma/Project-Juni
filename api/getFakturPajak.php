...
if ($_SERVER['REQUEST_METHOD'] !== 'GET') {
    ...
    try {
        ...
        while ($hasil = $master->fetch(PDO::FETCH_ASSOC)) {
            ...
            error_log("Raw status_urgent value: [" . ($rawUrgentValue ?? 'NULL') . "]");
            $isUrgent = false;
            $urgentType = 'normal'; // Default
            if (!is_null($rawUrgentValue)) {
                $urgentValue = strtolower(trim($rawUrgentValue));
                if ($urgentValue === 'urgent') {
                    $isUrgent = true;
                    $urgentType = 'urgent';
                } elseif ($urgentValue === 'nanti') {
                    // treat 'nanti' as NON-urgent for numbering, but keep its type for UI/styling.
                    $isUrgent = false;
                    $urgentType = 'nanti';
                } else {
                    // any other value -> keep original text but mark as normal type
                    $urgentType = 'normal';
                }
            }

            // Normalized status string for front-end (always present)
            if ($urgentType === 'urgent') {
                $normalizedStatus = 'URGENT';
            } elseif ($urgentType === 'nanti') {
                $normalizedStatus = 'NANTI';
            } else {
                $normalizedStatus = $rawUrgentValue ? strtoupper(trim($rawUrgentValue)) : null;
            }
            ...
            $rowData = [
                ...
                "status_tfk" => $hasil['status_tfk'], // <-- added: expose local invoice status (Revisi etc.)
                "id_tfk" => $hasil['id_tfk'],
                "jenis" => $jenis, // Set normalized jenis value
                "status_urgent" => $normalizedStatus, // <- tambahkan agar front-end bisa membaca
                "action" => [
            ...
            // Default visual flags
            $visualFlagType = null;
            $visualFlagColor = null;
            $visualLabel = null;
            $priority = "normal";

            if ($isUrgent) {
                // urgent item
                $visualFlagType = "red_flag";
                $visualFlagColor = "red";
                $visualLabel = "URGENT";
                $priority = "high";
                ...
                $rowData["display_no"] = (string)$urgent_start;
                $rowData["urgent"] = true;
                $rowData["urgent_flag"] = true;
                $rowData["urgent_type"] = $urgentType;
                $rowData["flag_type"] = $visualFlagType;
                $rowData["flag_color"] = $visualFlagColor;
                $rowData["urgent_label"] = $visualLabel;
                ...
                // normal or 'nanti' -> ikut nomor biasa (nomor global/normal)
                if ($urgentType === 'nanti') {
                    $visualFlagType = "orange_flag";
                    $visualFlagColor = "orange";
                    $visualLabel = "NANTI";
                    $priority = "medium";
                }
