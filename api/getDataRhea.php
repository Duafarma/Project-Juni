...
try {
    ...
    error_log("API getDataRhea validation error: " . $e->getMessage());
}
...
try {
    // 1. Total Sales (year to date) - filtered by RHEA MP0000000004
    $stmt = $conn->prepare("SELECT SUM(A.total_tfk) AS total 
        ...
        WHERE YEAR(A.tgl_tfk) = :tahun 
        AND P.nama_p = 'MP0000000004'");
    ...
    // 6. Unpaid Sales - filtered by RHEA MP0000000004
    $limit_outlet = $data->sistem('limit_outlet');
    $stmt = $conn->prepare("SELECT COUNT(DISTINCT A.id_tfk) AS total 
        FROM transaksi_faktur_pim AS A 
        LEFT JOIN transaksi_fakturdetail_pim AS D ON A.id_tfk = D.id_tfk 
        LEFT JOIN produk AS P ON D.id_pro = P.id_pro 
        WHERE A.status_tfk != 'Lunas' 
        AND (TIMESTAMPDIFF(DAY, :tanggal, A.tgl_limit) <= :limit 
        OR TIMESTAMPDIFF(DAY, A.tgl_limit, :tanggal) > 0) 
        AND P.nama_p = 'MP0000000004'");
    $stmt->bindParam(':tanggal', $tanggal, PDO::PARAM_STR);
    ...
    // 12. Top Products Bulan Lalu (Desember 2025) - filtered by RHEA MP0000000004
    ...
    // 13. Top Products Bulan Berjalan (Januari 2026) - filtered by RHEA MP0000000004
    $stmt = $conn->prepare("SELECT P.id_pro, P.nama_pro, K.nama_kpr, SUM(D.jumlah_tfd) AS qty 
        FROM transaksi_fakturdetail_pim AS D 
        LEFT JOIN produk AS P ON D.id_pro = P.id_pro 
        LEFT JOIN kategori_produk AS K ON P.id_kpr = K.id_kpr 
    ...
    for ($m = 1; $m <= 12; $m++) {
        ...
        // Data for 2024 - filtered by RHEA MP0000000004
        $month_key = $tiga . '-' . str_pad($m, 2, '0', STR_PAD_LEFT);
        $stmt = $conn->prepare("SELECT COALESCE(SUM(A.total_tfk), 0) AS total 
            FROM transaksi_faktur_pim AS A 
            LEFT JOIN transaksi_fakturdetail_pim AS D ON A.id_tfk = D.id_tfk 
            LEFT JOIN produk AS P ON D.id_pro = P.id_pro 
            WHERE LEFT(A.tgl_tfk, 7) = :month_key 
            AND P.nama_p = 'MP0000000004'");
        ...
        // Data for 2025 - filtered by RHEA MP0000000004
        ...
        // Data for 2026 - filtered by RHEA MP0000000004
    ...
    error_log("API getDataRhea query error: " . $e->getMessage());
