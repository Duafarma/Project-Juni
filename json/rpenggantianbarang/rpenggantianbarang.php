<?php
	...
	if($valid==false){
		...
		$no		= $mulai;
		$jumlah	= $conn->query(
                                "SELECT COUNT(A.id_tfd) AS total
                                 FROM transaksi_faktur_penggantian_barang_detail AS A LEFT JOIN transaksi_faktur_penggantian_barang AS B ON A.id_tfk=B.id_tfk
                                  WHERE A.id_tfd!='' $outlet $produk $tgl1 $tgl2")->fetch(PDO::FETCH_ASSOC);
		...
		$master	= $conn->prepare("SELECT 
                                            ...
                                            B.kode_tfkk DESC LIMIT :mulai, :maxi");
		...
		while($hasil= $master->fetch(PDO::FETCH_ASSOC)){
			...
			$tabel	.= '<tr>
                            ...
                            <td>'.$data->angka($hasil['jumlah_tfd']).'</td>
                        </tr>';
		}
