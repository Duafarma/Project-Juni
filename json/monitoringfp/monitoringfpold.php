<?php
    ...
    if($valid==false){
        ...
        ) AND A.upload_f_pajak = 'belum'";  // This filter is correctly set
        ...
        $cabangCurrentNormalNumber = [];
            ...
            foreach ($all_cabang as $cabang) {
                ...
                if (isset($api_data['data']) && is_array($api_data['data'])) {
                    ...
                    // PERBAIKAN: Normalisasi status urgent dengan lebih robust
                    $isUrgent = false;
                    
                    // Cek berbagai kemungkinan format status urgent
                    if (isset($row['urgent_flag']) && $row['urgent_flag'] === true) {
                        $isUrgent = true;
                    } 
                    elseif (isset($row['status_urgent'])) {
                        $urgentValue = strtolower(trim($row['status_urgent']));
                        $isUrgent = ($urgentValue == 'urgent' || $urgentValue == '1' || $urgentValue == 'true' || $urgentValue == 'yes');
                    }
                    
                    // Standarisasi status urgent
                    $row['urgent_flag'] = $isUrgent;
                    $row['status_urgent'] = $isUrgent ? 'urgent' : 'normal';
                    $row['flag_type'] = $isUrgent ? "red_flag" : null;
                    ...
                    // Proses penomoran berdasarkan status urgent yang sudah dinormalisasi
                    if ($isUrgent) {
                        $row['no'] = 'U' . $globalUrgentNumber++;
                        $row['original_no'] = $row['no']; // simpan asli untuk referensi
                    } else {
                        // Penomoran berurutan
    ...
    UNION ALL
            ...
            while ($hasil = $master->fetch(PDO::FETCH_ASSOC)) {
                ...
                // Ketika melakukan pengecekan urgent - MODIFIED TO USE REGULAR NUMBERS
                if (isset($hasil['status_urgent']) && strtolower(trim($hasil['status_urgent'])) == 'urgent') {
                    error_log("LOCAL DATA - URGENT DETECTED for invoice: " . $hasil['kode_tfk'] . ", numbering as: " . $no_urgent);
                    // Data urgent dengan format nomor reguler (bukan U-prefix)
                    $rowData["no"] = $no_urgent;
                    $rowData["display_no"] = (string)$no_urgent; // Plain string number
                    // Add urgent flag indicators with standard red flag
                    $rowData["urgent_flag"] = true;
                    $rowData["flag_type"] = "red_flag";
                    $dataUrgent[] = $rowData;
                    $no_urgent++;
                } else {
                    error_log("LOCAL DATA - NOT URGENT for invoice: " . $hasil['kode_tfk']);
                    // Add non-urgent flag indicators - match API implementation
                    $rowData["no"] = $no_normal;
                    $rowData["display_no"] = (string)$no_normal;
                    $rowData["urgent_flag"] = false;
                    $rowData["flag_type"] = null;
                    // Store normal data for processing after urgent items
                    $dataNormal[] = $rowData;
                    $no_normal++;
                }
            }
