<?php
	...
	if($valid==false){
		...
		$master	= $conn->prepare("
		                         ...
		                         AND B.nama_p IN ('MP0000000001','MP0000000005', 'MP0000000004')
		                         ORDER BY B.nama_pro ASC");
		$master->bindParam(':active', $active, PDO::PARAM_STR);
		$master->execute();
		while($hasil= $master->fetch(PDO::FETCH_ASSOC)){
			$no++;
			$submit	= (in_array($hasil['status'],array('belum so'))) ?  : (in_array($hasil['status'],array('proses revisi'))) ? '<a href="#modal1" onclick="crud(\'stockopname\', \'updateStatusDone\', \''.$hasil['id_psd'].'\')" data-toggle="modal"><span class="badge badge-success"><i class="fa fa-check"></i></span></a>' : '';
