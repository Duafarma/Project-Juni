<?php
	...
	if($valid==false){
		...
		while($hasil= $master->fetch(PDO::FETCH_ASSOC)){
			$no++;
			$view	= ($data->akses($admin, $menu, 'A.read_status')==='Active') ? '<a href="'.$sistem.'/transferretur/v/'.$hasil['id_ttr'].'"><span class="badge badge-warning"><i class="fa fa-search"></i></span></a> <a target="_blank" href="'.$sistem.'/laporan/xps/transferretur/transferretur.php?key='.$hasil['id_ttr'].'"><span class="badge badge-success"><i class="fa fa-print"></i></span></a>' : '';
			$delete	= ($data->akses($admin, $menu, 'A.delete_status')==='Active' && $hasil['status_ttr'] == 'Process') ? ' <a href="#modal1" onclick="crud(\'transferretur\', \'delete\', \''.$hasil['id_ttr'].'\')" data-toggle="modal"><span class="badge badge-danger"><i class="fa fa-trash"></i></span></a>' : '';
			
			$tabel	.= '
				<tr>
					<td><center>'.$no.'</center></td>
					<td><a href="#modal1" onclick="crud(\'transferretur\', \'tutup\', \''.$hasil['id_ttr'].'\')" data-toggle="modal">'.$hasil['kode_ttr'].'</a></td>
					<td>'.$hasil['tgl_ttr'].'</td>
