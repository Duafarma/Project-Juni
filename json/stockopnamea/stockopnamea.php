<?php
	...
	if($valid==false){
	    ...
	    $active	= 'Active';
		$tabel	= '';
		$master	= $conn->prepare("SELECT  A.id, A.id_psd, A.id_pro, A.nama_pro, A.qty, A.qty_so, B.harga_phg FROM total_inventory AS A LEFT JOIN produk_harga AS B ON A.id_pro=B.id_pro WHERE A.selisih<0 AND B.status_phg=:active GROUP BY A.id_pro");
	   	$master->bindParam(':active', $active, PDO::PARAM_STR);
		$master->execute();
		while($hasil= $master->fetch(PDO::FETCH_ASSOC)){
			
			$selisi	= $data->selisih($hasil['id_pro']);
			$b= $hasil['harga_phg'];
			$selisih = ($b * $selisi);
