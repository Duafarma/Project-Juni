<?php
	...
	if($valid==false){
		...
		while($hasil= $master->fetch(PDO::FETCH_ASSOC)){
			$no++;
			$submit	= (in_array($hasil['status'],array('belum so'))) ?  : (in_array($hasil['status'],array('proses revisi'))) ? '<a href="#modal1" onclick="crud(\'stockopnamedpe\', \'updateStatusDone\', \''.$hasil['id_psd'].'\')" data-toggle="modal"><span class="badge badge-success"><i class="fa fa-check"></i></span></a>' : '';
