...
<?php
	...
	$read	= $conn->prepare("SELECT A.sj_tfk, A.po_tfk, A.tglsj_tfk, B.nama_out, B.npwp_out, B.resmi_out, C.pengiriman_ola FROM transaksi_faktur_konsinyasi AS A LEFT JOIN outlet AS B ON A.id_out=B.id_out LEFT JOIN outlet_alamat AS C ON B.id_out=C.id_out WHERE A.id_tfk=:kode");
...
?>
    <head>
        ...
        <title>Surat Jalan Konsinyasi</title>
...
<div align="right" style="font-size:10px; font-family:Arial, Helvetica, sans-serif;"><?php echo substr($data->sistem('alamat_sis'),0,69); ?></div>
    	...
    	<table class="tabel">
            ...
            <?php
				$nomor	= 1;
				$master	= $conn->prepare("SELECT A.jumlah_tfd, A.terjual_tfd, A.sisa_tfd, B.kode_pro, B.nama_pro, B.berat_pro, C.nama_kpr, D.nama_spr, E.no_bcode, E.tgl_expired FROM transaksi_fakturdetail_konsinyasi AS A LEFT JOIN produk AS B ON A.id_pro=B.id_pro LEFT JOIN kategori_produk AS C ON B.id_kpr=C.id_kpr LEFT JOIN satuan_produk AS D ON B.id_spr=D.id_spr LEFT JOIN produk_stokdetail_konsinyasi AS E ON A.id_psd=E.id_psd WHERE A.id_tfk=:kode ORDER BY B.nama_pro ASC");
			...
			<table class="tabel" style="font-size:10px;">
            	<thead>
                	<tr>
                    	...
                    	<th width="20%"><div>PT. Dua Farma Mahakarsa</div></th>
                    	<th width="20%"><div>PT. Dua Farma Mahakarsa</div></th>
						<th width="20%"><div><center><?php echo($view['nama_out']); ?></center></div></th>
                    </tr>
                </thead>
                <tbody>
                	<tr>
			...
			<table style="font-size:100px;">
            	<thead>
                	<tr >
                    	...
                    	<th width="20%"><div></div></th>
                    	<th width="20%" ><div><center></center></div></th>
                    	<th width="20%"><div></div></th>
						<th width="20%"><div></div></th>
                    </tr>
                </thead>
                <tbody>
                	<tr>
                        ...
                        <td height="0" style="vertical-align:bottom;">
						<div style="font-size:100px; border:solid px" ><center></center></div>
                        <div style="font-size:15px;"><center> </center></div>
						</td>
                    	<td>
						<td height="0" style="vertical-align:bottom;">
