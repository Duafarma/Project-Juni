...
<?php
	...
	header("content-disposition:attachment; filename=report_dokumen_PIM.xls");
...
?>
    ...
    <body>
		<table>
        	<tr>
            	<th colspan="18">REPORT DOKUMEN FAKTUR PIM</th>
            </tr>
            <tr>
            	<td colspan="18"></td>
            </tr>
    	...
    	<table border="1">
            ...
            <?php
				...
				$qmaster = "SELECT A.id_tfk, 
		                    ...
		                    F.tanggal_tf, 
		                    D.nama_adm 
		                    FROM transaksi_faktur_pim AS A LEFT JOIN 
		                    outlet AS B ON A.id_out=B.id_out LEFT JOIN 
		                    transaksi_faktur_kirim_f AS C ON A.id_tfk=C.id_tfk LEFT JOIN 
		                    adminz AS D ON C.id_adm=D.id_adm 
		                    LEFT JOIN dokumen_tf_balik_detail AS E ON A.id_tfk = E.no_faktur
		                    LEFT JOIN dokumen_tf_balik AS F ON E.id_tfb = F.id_tfb
		                    WHERE (A.kode_tfk LIKE '%$cari%' OR B.nama_out LIKE '%$cari%') $tgl1 $tgl2 ORDER BY A.tgl_tfk DESC, A.kode_tfk DESC";
