...
<?php
	...
	header("content-disposition:attachment; filename=report_penjualan_faktur_pim.xls");
    	...
    	<table border="1">
            ...
            <?php
				...
				$qmaster ="SELECT 
				                    ...
				                    F.pengiriman_ola, 
				                    H.nama_rkb 
				                    FROM transaksi_fakturdetail_pim AS A LEFT JOIN 
				                    transaksi_faktur_pim AS B ON A.id_tfk=B.id_tfk LEFT JOIN 
				                    produk AS C ON A.id_pro=C.id_pro LEFT JOIN 
				                    outlet AS D ON B.id_out=D.id_out  LEFT JOIN 
				                    produk_stokdetail AS E ON A.id_psd=E.id_psd LEFT JOIN 
				                    outlet_alamat AS F ON D.id_out=F.id_out LEFT JOIN 
				                    kategori_outlet AS G ON D.id_kot=G.id_kot LEFT JOIN 
				                    regional_kabupaten AS H ON F.id_rkb=H.id_rkb 
				                    WHERE A.id_tfd!='' $outlet $produk $tgl1 $tgl2 AND D.status_out=:status 
