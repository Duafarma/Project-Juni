    ...
    <body>
    	...
    	<table border="1">
            ...
            <?php
				$nomor	= 1;
				$status	= 'Active';
				$qmaster = "SELECT A.jumlah_tfd, A.harga_tfd, A.diskon_tfd, I.nama_principle, A.total_tfd,B.ket,B.ket, B.pajak_tfk,B.pajak_tfkt, B.kode_tfk, B.tgl_tfk, B.po_tfk, B.tglpo_tfk, B.tgl_limit, B.status_tfk,B.status_tfkkb, B.status_tfkkf, TIMESTAMPDIFF(DAY, B.tgl_limit, :tanggal) AS jarak,  C.nama_pro, C.kategori_obat, C.kode_pro,C.kode_produk_jadi, D.nama_out,D.kode_out,D.kode_rs, D.id_kot, D.ofcode_out, E.no_bcode, E.tgl_expired, E.gudang,F.pengiriman_ola, G.kode_kot, H.nama_rkb FROM transaksi_fakturdetail AS A LEFT JOIN transaksi_faktur AS B ON A.id_tfk=B.id_tfk LEFT JOIN produk AS C ON A.id_pro=C.id_pro LEFT JOIN outlet AS D ON B.id_out=D.id_out  LEFT JOIN produk_stokdetail AS E ON A.id_psd=E.id_psd LEFT JOIN outlet_alamat AS F ON D.id_out=F.id_out LEFT JOIN kategori_outlet AS G ON D.id_kot=G.id_kot LEFT JOIN regional_kabupaten AS H ON F.id_rkb=H.id_rkb LEFT JOIN master_principle AS I ON C.nama_p = I.id_mp WHERE A.id_tfd!='' $outlet $produk $tgl1 $tgl2 AND D.status_out=:status ORDER BY B.tgl_tfk DESC, CAST(B.kode_tfk AS UNSIGNED) DESC";
			...
			?>
				<tr>
                	...
                	<td><center><?php echo($hasil['ket']); ?></center></td>
