...
<?php
	...
	$read	= $conn->prepare("SELECT A.sj_tfk, A.po_tfk,A.cito,A.ccp, A.tglsj_tfk, B.nama_out, B.npwp_out, B.resmi_out, C.pengiriman_ola FROM transaksi_faktur_pim AS A LEFT JOIN outlet AS B ON A.id_out=B.id_out LEFT JOIN outlet_alamat AS C ON B.id_out=C.id_out WHERE A.id_tfk=:kode");
	$read->bindParam(':kode', $kode, PDO::PARAM_STR);
	$read->execute();
	$view	= $read->fetch(PDO::FETCH_ASSOC);
	$submit = (in_array($view['cito'], array('cito'))) ? '<div style="border:solid 1px">CITO</div>' : '';
$ccp = (in_array($view['ccp'], array('ada'))) ? '<div style="border:solid 1px">CCP</div>' : '';
