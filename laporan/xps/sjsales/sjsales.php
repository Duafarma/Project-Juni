...
<?php
	...
	$read	= $conn->prepare("SELECT A.sj_tfk, A.po_tfk,A.cito,A.ccp, A.tglsj_tfk, B.nama_out, B.npwp_out, B.resmi_out, C.pengiriman_ola FROM transaksi_faktur AS A LEFT JOIN outlet AS B ON A.id_out=B.id_out LEFT JOIN outlet_alamat AS C ON B.id_out=C.id_out WHERE A.id_tfk=:kode");
	$read->bindParam(':kode', $kode, PDO::PARAM_STR);
	$read->execute();
	$view	= $read->fetch(PDO::FETCH_ASSOC);
	$submit = (in_array($view['cito'], array('cito'))) ? '<div style="border:solid 1px">CITO</div>' : '';
	$ccp = '';
	$master = $conn->prepare("SELECT C.ccp FROM transaksi_fakturdetail AS A 
	...
	while($hasil = $master->fetch(PDO::FETCH_ASSOC)){
		if($hasil['ccp'] == 'ya'){
			$ccp = '<div style="border:solid 1px">CCP</div>';
			break;
		}
	}
...
<div align="right" style="font-size:10px; font-family:Arial, Helvetica, sans-serif;"><?php echo substr($data->sistem('alamat_sis'),0,69); ?></div>
    	...
    	<table class="tabel">
            ...
            <?php
				...
				$stotal	= 0;
				$nomor	= 1;
				$master	= $conn->prepare("SELECT A.jumlah_tfd, A.harga_tfd, A.diskon_tfd, B.no_bcode, B.tgl_expired, C.kode_pro, C.nama_pro, C.berat_pro, C.ccp, D.nama_kpr, E.nama_spr FROM transaksi_fakturdetail AS A LEFT JOIN produk_stokdetail AS B ON A.id_psd=B.id_psd LEFT JOIN produk AS C ON B.id_pro=C.id_pro LEFT JOIN kategori_produk AS D ON C.id_kpr=D.id_kpr LEFT JOIN satuan_produk AS E ON C.id_spr=E.id_spr WHERE A.id_tfk=:kode");
				$master->bindParam(':kode', $kode, PDO::PARAM_STR);
				$master->execute();
				while($hasil= $master->fetch(PDO::FETCH_ASSOC)){
			...
			<table style="font-size:100px;">
            	<thead>
                	<tr >
                    	...
                    	<th width="20%"><div></div></th>
                    	<th width="20%" ><div><center><?php echo($submit); ?></center></div></th>
                    	<th width="20%"><div></div></th>
						<th width="20%"><div></div></th>
                    </tr>
                </thead>
                <tbody>
                	<tr>
                        ...
                        <td height="0" style="vertical-align:bottom;">
						<div style="font-size:100px; border:solid px" ><center><?php echo($ccp); ?></center></div>
                        <div style="font-size:15px;"><center> </center></div>
						</td>
                    	<td>
						<td height="0" style="vertical-align:bottom;">
