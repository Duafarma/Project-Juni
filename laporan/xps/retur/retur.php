...
<?php
	...
	$read	= $conn->prepare("SELECT A.no_retur, A.tanggal, A.keterangan, B.nama_out, C.pengiriman_ola FROM retur AS A LEFT JOIN outlet AS B ON A.id_out=B.id_out LEFT JOIN outlet_alamat AS C ON B.id_out=C.id_out WHERE A.id_r=:kode");
...
?>
    ...
    <body>
		...
		<hr class="garis" />
        <div style="text-align:center; font-weight:bold; margin:10px 0px 10px 0px;">Surat Retur</div>
        <div style="text-align:center; font-weight:bold;">NO. Retur : <?php echo($view['no_retur']); ?></div>
        <div style="text-align:center; font-weight:bold; margin-bottom:20px;">Tanggal : <?php echo($date->tgl_indo($view['tanggal'])); ?></div>
    	...
    	<table class="tabel">
            ...
            <?php
				$nomor	= 1;
				$master	= $conn->prepare("SELECT A.jumlah, B.nama_pro, C.nama_kpr, C.satuan_kpr FROM retur_detail AS A LEFT JOIN produk AS B ON A.id_pro=B.id_pro LEFT JOIN kategori_produk AS C ON B.id_kpr=C.id_kpr WHERE A.id_r=:kode");
			...
			?>
            	<tr>
                	...
                	<td><center><?php echo($hasil['jumlah']); ?></center></td>
        ...
        <table width="100%">
        	<tr>
            	<td width="30%">Penerimaan Retur</td>
            	<td width="40%"></td>
