...
<?php
	...
	$read	= $conn->prepare("SELECT A.kode_tfk, A.tgl_tfk, A.tgl_limit, A.po_tfk, TIMESTAMPDIFF(DAY, A.tgl_tfk, A.tgl_limit) AS jarak,B.resmi_out, B.nama_out, B.npwp_out, C.pengiriman_ola FROM transaksi_faktur AS A LEFT JOIN outlet AS B ON A.id_out=B.id_out LEFT JOIN outlet_alamat AS C ON B.id_out=C.id_out WHERE A.id_tfk=:kode");
    ...
    <body>
		<div>
        	...
        	<div align="right" style="font-size:10px;">CDOB : <?php echo($data->sistem('cdob_sis')); ?></div>
        	 <div align="right" style="font-size:10px;">CDOB CCP : CDOB4998/S/1-1941/03/2024 </div>
        </div>
    	<br />
        ...
        <div style="width:100%;">
            ...
            <div style="width:45%; display:inline-block; float:right;">
                ...
                <table width="80%" style="font-family:Calibri Light, Helvetica, sans-serif; font-size:12px;">
					...
					<tr>
                        <td><div align="left">Dasar Pengenaan Pajak</div></td>
                        <td></td>
                        <td><div align="right"><span style="float:left;">Rp.</span><?php echo($data->angka($dpp)); ?></div></td>
                    </tr>
                    <tr>
