...
<?php
	...
	$read	= $conn->prepare("SELECT A.kode_tfk, A.tgl_tfk, A.total_tfk, A.tgl_limit, A.po_tfk, TIMESTAMPDIFF(DAY, A.tgl_tfk, A.tgl_limit) AS jarak, B.resmi_out, B.nama_out, B.npwp_out, C.pengiriman_ola FROM transaksi_faktur AS A LEFT JOIN outlet AS B ON A.id_out=B.id_out LEFT JOIN outlet_alamat AS C ON B.id_out=C.id_out WHERE A.id_tfk=:kode");
