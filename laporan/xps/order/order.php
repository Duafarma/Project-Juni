...
<?php
	...
	$read	= $conn->prepare("SELECT A.kode_tor, A.tgl_tor, A.id_sup, B.nama_sup, C.alamat_sal FROM transaksi_order AS A LEFT JOIN supplier AS B ON A.id_sup=B.id_sup LEFT JOIN supplier_alamat AS C ON B.id_sup=C.id_sup WHERE A.id_tor=:kode");
    	...
    	<table class="tabel">
        	<thead>
            	<tr>
                    ...
                    <?php if($view['id_sup'] == 'SUP1738590257'): ?>
                    <th><div align="center">HARGA/SATUAN</div></th>
                    <?php endif; ?>
            ...
            <?php
				$nomor	= 1;
				$master	= $conn->prepare("SELECT A.jumlah_tod, B.nama_pro, C.nama_kpr, C.satuan_kpr, D.harga_phg FROM transaksi_orderdetail AS A LEFT JOIN produk AS B ON A.id_pro=B.id_pro LEFT JOIN kategori_produk AS C ON B.id_kpr=C.id_kpr LEFT JOIN produk_harga AS D ON B.id_pro=D.id_pro AND D.status_phg='Active' WHERE A.id_tor=:kode");
			...
			?>
            	<tr>
                	...
                	<?php if($view['id_sup'] == 'SUP1738590257'): ?>
                	<td><center><?php echo('Rp '.number_format($hasil['harga_phg'],0,',','.')); ?></center></td>
                	<?php endif; ?>
                	<td><center></center></td>
                </tr>
			<?php
