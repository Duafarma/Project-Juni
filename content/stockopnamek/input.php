        ...
        <div class="row row-sm">
            <div class="col-sm-12">
				...
				<table class="tabel">
                	<thead>
                        <tr>
                              <th colspan="5"><center>STOK INVENTORY</center></th>
                    ...
                    <?php
						$active	= 'Active';
                        $master	= $conn->prepare("SELECT A.id_psd, A.no_bcode, A.id_pro, A.status_barang,A.status, A.tgl_expired, A.gudang,A.status, A.tgl_psd, A.qty_so, A.sisa_psd,A.status, B.nama_pro, B.berat_pro,B.minstok_pro, C.harga_phg, C.hargap_phg, E.nama_spr FROM produk_stokdetail AS A LEFT JOIN produk AS B ON A.id_pro=B.id_pro LEFT JOIN produk_harga AS C ON B.id_pro=C.id_pro LEFT JOIN kategori_produk AS D ON B.id_kpr=D.id_kpr LEFT JOIN satuan_produk AS E ON B.id_spr=E.id_spr WHERE (A.sisa_psd > 0 OR (A.sisa_psd = 0 AND A.qty_so > 0)) AND C.status_phg=:active  ORDER BY B.nama_pro ASC");
