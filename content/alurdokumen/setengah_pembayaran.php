    ...
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Dokumen Sebagian Pembayaran </li>
            </ol>
        </nav>
        <h4 class="content-title">Pembayaran Sebagian </h4>
    </div>
</div>
<?php
        ...
        <table class="table table-hover mg-b-0">
            <thead>
                <tr>
                    ...
                    <th><center>Sudah Di bayar</center></th>


                    <th><center>Sisa tagihan</center></th>
                </tr>
            </thead>
            <tbody>
            <?php
				...
				$qMaster = "SELECT
                                ...
                                B.nama_out,
                                C.jumlah_pfk
                                -- SUM(C.jumlah_pfk) AS bayar
                            FROM
                                transaksi_faktur AS A
					        LEFT JOIN outlet AS B ON
                                A.id_out = B.id_out
                            LEFT JOIN pembayaran_faktur AS C ON
                                A.id_tfk = C.id_tfk
                            WHERE
                                 A.tgl_tfk=:kode AND A.status_tfk='Bayar'";
			...
			?>
				<tr>
                    ...
                    <td><center><?php echo($data->angka($hasil['jumlah_pfk'])); ?></center></td>
                    <td><center><?php echo($data->angka($hasil['total_tfk'] - $hasil['jumlah_pfk'])); ?></center></td>
