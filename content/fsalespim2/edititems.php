<?php
	...
	$read	= $conn->prepare("SELECT sj_tfk, id_out, kode_tfk, status_tfk FROM transaksi_faktur_pim WHERE id_tfk=:code");
            ...
            <div class="col-sm-12">
                ...
                <table class="tabeltransaksi">
                    ...
                    <tfoot>
                        ...
                        <!-- FIX: tambahkan pdpp supaya getproductsalespim2() tidak membaca undefined -->
                        <tr>
                            <td></td>
                            <td colspan="8"><div align="right"><b>DPP</b></div></td>
                            <td><input type="text" name="pdpp" id="pdpp" class="inputtotal" onkeyup="angka(this)" placeholder="0" readonly="readonly" /></td>
                            <td></td>
                        </tr>
                ...
                <!-- FIX: alias cart supaya cart() di fazlurr.js tidak null -->
                ...
                <!-- yang memang dipakai modul ini
                <input type="hidden" name="cartaddsalespim2" id="cartaddsalespim2" value="" readonly="readonly" /> -->

                <input type="hidden" name="jumaddsalespim2" id="jumaddsalespim2" value="0" readonly="readonly" />
                <input type="hidden" name="dataaddsalespim2" id="dataaddsalespim2_hidden" value="" readonly="readonly" />
            </div>
        </div>
        <div class="clearfix mg-t-25 mg-b-25"></div>
        <div class="row row-sm">
            <div class="col-sm-12">
                <a href="<?php echo("$sistem/fsalespim2"); ?>" title="Batal">
...
window.openProductModal = function(nomor, outlet) {
    ...
    var modalUrl = usuper + "/modal/addsalespim2/addsalespim2.php";
...
$("#loadExistingItems").click(function(){
    ...
    $.ajax({
        url: usuper + "/ajax/detailsalesfsalespim2/detailsalesfsalespim2.php",
        type: "POST",
        async: true,
        dataType: "json",
        cache: false,
        ...
        success: function(data) {
            ...
            if (data && typeof data === 'object') {
                if (data.error) {
                    $("#loadingitems").html('<td colspan="11"><center class="text-danger">Error: ' + data.error + '</center></td>');
                    console.error('Server error:', data.error);
                    return;
                }
                
                $("#dataaddsalespim2").html(data.tabel);
                $("#jumaddsalespim2").val(data.jumlahitem);
                $("#pstotal").val(data.subtotal);

                // Calculate PPN and total
                var subtotal = parseInt(String(data.subtotal || '0').replace(/\./g, '')) || 0;
                var ppn = Math.round(subtotal * 0.11);
                var total = subtotal + ppn;

                $("#pdpp").val(addCommas(ppn));
                $("#pppn").val(addCommas(ppn));
                $("#pgtotal").val(addCommas(total));

                setTimeout(function() {
                    $("input[name='jumlah[]']").each(function(index) {
                        updateStokTersedia(index + 1);
                    });
                }, 500);
            } else {
                $("#loadingitems").html('<td colspan="11"><center class="text-danger">Response tidak valid</center></td>');
                console.error('Invalid response format:', data);
            }
        },
        error: function(xhr, status, error) {
            $("#loadingitems").html('<td colspan="11"><center class="text-danger">Gagal memuat data: ' + error + '</center></td>');
...
function recalculateTotal() {
    ...
    // Samakan: pdpp ada supaya getproductsalespim2() tidak error
    $("#pdpp").val(addCommas(ppn));
...
function deletefsalesitem(nomor, idTfd) {
    if (confirm('Apakah Anda yakin ingin menghapus item ini?')) {
        ...
        // Update jumlah item
        var currentCount = parseInt($("#jumaddsalespim2").val()) || 0;
        $("#jumaddsalespim2").val(currentCount - 1);
    }
}
...
$("#formsalespnp").submit(function(e){
    ...
    var itemCount = parseInt($("#jumaddsalespim2").val()) || 0;
    if (itemCount === 0) {
    ...
    $.ajax({
        url: usuper + "/modal/fsalespim2/action.php",
        type: "POST",
        async: true,
        dataType: "json",
        cache: false,
...
$("#addNewItem").click(function(){
    addmaximal('addsalespim2', 'outlet', 200);
});
...
$(document).ready(function() {
    ...
    if (keteranganRevisi) {
        ...
        swal("Error", "Keterangan revisi tidak ditemukan. Silakan klik tombol Edit Item lagi dari halaman Faktur Penjualan.", "error").then(function() {
            window.location.href = usuper + '/fsalespim2';
        });
        return;
    }
