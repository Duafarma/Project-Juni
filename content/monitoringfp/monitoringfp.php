    ...
    window.loadMonitoringFP = function(resetNumbering) {
        ...
        pendingRequest = $.ajax({
            ...
            success: function(response) {
                ...
                if (!response.success) {
                    $('#isitabel').html('<tr><td colspan="9" class="text-center text-danger">'+
                        '<i class="fa fa-exclamation-triangle mr-2"></i>' + response.message +
                        '</td></tr>');
                    $('#paginasi').html('');
                    
                    // Tambahkan notifikasi error di atas tabel
                    if (response.message.includes('cabang')) {
                        $('.table-scroll-top').before(
                            '<div class="alert alert-danger mb-3">' +
                            '<h5><i class="fa fa-exclamation-circle mr-2"></i>Kesalahan Koneksi Cabang</h5>' +
                            '<p>' + response.message + '</p>' +
                            '<p>Kemungkinan penyebab:</p>' +
                            '<ul>' +
                            '<li>Konfigurasi URL API cabang tidak benar</li>' +
                            '<li>Server cabang sedang tidak aktif</li>' +
                            '<li>Masalah jaringan antara server utama dan cabang</li>' +
                            '</ul>' +
                ...
                if (response && response.success && Array.isArray(response.data)) {
                    ...
                    if (response.data.length > 0) {
                        response.data.forEach(function(row) {
                            ...
                            var urgentType = row.urgent_type || 'normal'; // Tambahkan urgent_type
                            
                            // Add urgent-row class if it's an urgent item
                            var rowClass = '';
                            if (isUrgent) {
                                if (urgentType === 'nanti') {
                                    rowClass = 'urgent-row-nanti'; // Class untuk orange
                                } else {
                                    rowClass = 'urgent-row'; // Class untuk red (urgent biasa)
                                }
                            }
                            html += '<tr' + (rowClass ? ' class="' + rowClass + '"' : '') + '>';
                            
                            // Display number column - warnai nomor berdasarkan status urgent (tetap tidak mewarnai nomor faktur di sini)
                            var numberClass = '';
                            if (isUrgent) {
                                if (urgentType === 'nanti') {
                                    numberClass = 'nanti-number'; // Use nanti-number for consistency with kode_tfk
                                } else {
                                    numberClass = 'urgent-number'; // Merah untuk urgent biasa
                                }
                            }
                            html += '<td align="center" class="' + numberClass + '">' + displayNumber + '</td>';
                            // Kode faktur hanya sekali, dengan flag urgent/nanti dan REVISI (biru) di nomor faktur
                            var isNanti = (row.urgent_type && String(row.urgent_type).toLowerCase() === 'nanti') ||
                                      (row.status_urgent && String(row.status_urgent).toLowerCase() === 'nanti');
                            var kodeHtml = (row.kode_tfk || '');
                            // Revisi harus mendapat prioritas warna biru pada nomor faktur
                            if (row.status_tfk && String(row.status_tfk).toLowerCase() === 'revisi') {
                                kodeHtml = '<span class="revisi-number" title="Status: REVISI">' + kodeHtml + '</span>';
                            } else if (isNanti) {
                                kodeHtml = '<span class="nanti-number" title="Status: NANTI">' + kodeHtml + '</span>';
                            }

                       // Tambahkan logo "R" kecil di sebelah nomor faktur
                       // hanya tampilkan logo "R" untuk status_tfk = 'revisi'
                       if (row.status_tfk && String(row.status_tfk).toLowerCase() === 'revisi') {
                           kodeHtml = kodeHtml + ' <span class="r-logo" title="R">R</span>';
                       }
        ...
        error: function(xhr, status, error) {
            $('#isitabel').html('<tr><td colspan="9" class="text-center text-danger">'+
                '<i class="fa fa-exclamation-triangle mr-2"></i>Terjadi kesalahan: ' + error +
                '<br>Periksa koneksi jaringan dan coba refresh halaman</td></tr>');
            $('#paginasi').html('');
        }
    });
    };
            ...
            if (response && response.success && Array.isArray(response.data)) {
                ...
                if (response.data.length > 0) {
                    response.data.forEach(function(row) {
                       ...
                       // hanya tampilkan logo "R" untuk status_tfk = 'revisi'
                       if (row.status_tfk && String(row.status_tfk).toLowerCase() === 'revisi') {
                          // tambahkan data-toggle dan title untuk bootstrap tooltip
                          kodeHtml = kodeHtml + ' <span class="r-logo" data-toggle="tooltip" data-placement="right" title="Status: REVISI" aria-label="Revisi">R</span>';
                       }
        ...
        success: function(response) {
            if (response.success && response.data && response.data.length > 0) {
                ...
                response.data.forEach(function(file) {
                    ...
                    html += '<div class="list-group-item list-group-itemaction py-2 d-flex justify-content-between align-items-center">';
                    html += '<div>';
...
/* Nanti number styling */
.nanti-number {
    color: #ffd735ff !important; /* Orange color */
    font-weight: bold;
}
...
.nanti-number { color: #ff8c00; font-weight: bold; } /* if not already present */
.revisi-number {
