<?php
    ...
    $header  = $conn->prepare("SELECT id_pp, nama_program, jenis_program, min_qty, diskon_persen FROM master_program_produk WHERE id_pp=:id");
    ...
    $outletSel = $conn->prepare("SELECT id_out FROM master_program_produk_outlet WHERE id_pp=:id");
    $outletSel->bindParam(':id', $id_edit, PDO::PARAM_INT);
    $outletSel->execute();
    $selectedOutlets = $outletSel->fetchAll(PDO::FETCH_COLUMN, 0);
    $details = $conn->prepare("SELECT B.id_ppd, B.id_pro, B.harga_program, C.kode_pro, C.nama_pro, C.berat_pro, D.nama_kpr, E.nama_spr FROM master_program_produk_detail AS B LEFT JOIN produk AS C ON B.id_pro=C.id_pro LEFT JOIN kategori_produk AS D ON C.id_kpr=D.id_kpr LEFT JOIN satuan_produk AS E ON C.id_spr=E.id_spr WHERE B.id_pp=:id ORDER BY C.nama_pro ASC");
    ...
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="<?php echo($sistem); ?>/master_program_produk">Master Program Produk</a></li>
...
<div class="content-body">
    <div class="component-section no-code">
        ...
        <input type="hidden" name="namamodal" id="namamodal" value="master_program_produk" readonly="readonly" />
                            ...
                            <tr id="traddorder<?php echo($nomor); ?>">
                                <td>
                                    <a href="#modal1" onclick="mproductmp(<?php echo($nomor); ?>)" data-toggle="modal">
                                        <div id="noproduct<?php echo($nomor); ?>">(<?php echo($row['kode_pro']); ?>) <?php echo($row['nama_pro']); ?></div>
                                    </a>
                                    <input type="hidden" name="product[]" id="product<?php echo($nomor); ?>" class="itemproduct" value="<?php echo($row['id_pro']); ?>" readonly="readonly" />
                                </td>
                                <td><div id="detailproduct<?php echo($nomor); ?>"><?php echo($detail); ?></div></td>
                                <td>
                                    <input type="text" name="harga[]" id="pharga<?php echo($nomor); ?>"
                                           class="inputangka form-control form-control-sm"
                                           onkeyup="angka(this)"
                                           value="<?php echo(number_format($row['harga_program'],0,',','.')); ?>,"
                                           placeholder="0" required="required" />
                                </td>
                                <td>
                                    <center>
                                        <a onclick="deleteorder(<?php echo($nomor); ?>)">
        ...
        <a href="<?php echo($sistem); ?>/master_program_produk">
...
function mproductmp(nomor){
    $.ajax({
        url     : usuper+"/modal/master_program_produk/showproduct.php",
        type    : "POST",
        async   : true,
        dataType: "text",
        cache   : false,
...
$(document).ready(function(){
    $("#formmenu").off('submit').on('submit', function(e){
        ...
        if(modal !== 'master_program_produk') return;
