    ...
    <div class="component-section no-code">
        ...
        <div class="row row-sm">
            <div class="col-sm-12">
                ...
                <input type="hidden" name="jumaddsalespim" id="jumaddsalespim" value="0" readonly="readonly" />
                <input type="hidden" name="dataaddsalespim" id="dataaddsalespim" value="" readonly="readonly" />
            </div>
        </div>
        <div class="clearfix mg-t-25 mg-b-25"></div>
        <div class="row row-sm">
            <div class="col-sm-12">
                <a href="<?php echo("$sistem/fsalespim"); ?>" title="Batal">
...
window.openProductModal = function(nomor, outlet) {
    ...
    var modalUrl = usuper + "/modal/addsalespim/addsalespim.php";
...
$("#loadExistingItems").click(function(){
    ...
    $.ajax({
        url: usuper + "/ajax/detailsalesfsalespim/detailsalesfsalespim.php",
        type: "POST",
        async: true,
        dataType: "json",
        cache: false,
        data: { "x": code },
        success: function(data) {
            $("#dataaddsalespim").html(data.tabel);
            $("#jumaddsalespim").val(data.jumlahitem);
...
function deletefsalesitem(nomor, idTfd) {
    if (confirm('Apakah Anda yakin ingin menghapus item ini?')) {
        ...
        // Update jumlah item
        var currentCount = parseInt($("#jumaddsalespim").val()) || 0;
        $("#jumaddsalespim").val(currentCount - 1);
    }
}
...
$("#formsalespnp").submit(function(e){
    ...
    var itemCount = parseInt($("#jumaddsalespim").val()) || 0;
    if (itemCount === 0) {
    ...
    $.ajax({
        url: usuper + "/modal/fsalespim/action.php",
        type: "POST",
        async: true,
        dataType: "json",
        cache: false,
...
$("#addNewItem").click(function(){
    addmaximal('addsalespim', 'outlet', 200);
});
...
$(document).ready(function() {
    ...
    if (keteranganRevisi) {
        ...
        swal("Error", "Keterangan revisi tidak ditemukan. Silakan klik tombol Edit Item lagi dari halaman Faktur Penjualan.", "error").then(function() {
            window.location.href = usuper + '/fsalespim';
        });
        return;
    }
