...
// Debug log
error_log("=== LOADPRODUCTS PENGGANTIAN ===");
...
try {
    ...
    if(count($products) > 0) {
        foreach($products as $hasil) {
            ...
            // Create an array of parameters for the function
            $params = array(
                $nomor,                    // param 1: nomor
            ...
            // Build onclick string with proper JSON-encoded string literals
            $onclickParts = array();
            foreach($params as $param) {
                // Use JSON encode to properly escape strings
                $onclickParts[] = json_encode((string)$param);
            }
            $onclickAttr = "getproductsales(" . implode(", ", $onclickParts) . ")";
            
            echo '<tr onclick="' . htmlspecialchars($onclickAttr, ENT_QUOTES, 'UTF-8') . '">';
        ...
        // Add pagination info as hidden div
    ...
    error_log("LoadProducts Penggantian Error: " . $e->getMessage());
