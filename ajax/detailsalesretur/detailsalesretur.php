<?php
	...
	$total	= 0;
	$active	= 'Active';
	$master	= $conn->prepare("SELECT A.id_i_r, A.jumlah_tsd, A.harga_tsd, A.diskon_tsd, A.total_tsd, B.id_pro, B.kode_pro, B.nama_pro, B.berat_pro, C.nama_kpr, C.satuan_kpr, D.nama_spr, E.gudang, E.no_bcode, E.ed FROM transaksi_salesdetailretur AS A LEFT JOIN produk AS B ON A.id_pro=B.id_pro LEFT JOIN kategori_produk AS C ON B.id_kpr=C.id_kpr LEFT JOIN satuan_produk AS D ON B.id_spr=D.id_spr LEFT JOIN inventory_retur AS E ON A.id_i_r=E.id_i_r WHERE A.id_tsl=:kode");
	$master->execute();
	while($hasil= $master->fetch(PDO::FETCH_ASSOC)){
		$tabel	.= '<tr id="traddfaktur'.$nomor.'">
						...
						<td>
						'.$hasil['jumlah_tsd'].'
						<input type="hidden" name="jumlah[]" id="pjumlah'.$nomor.'" class="inputangka" value="'.$hasil['jumlah_tsd'].'" onchange="hitungsalesretur('.$nomor.')" onkeyup="angka(this)" placeholder="0" readonly="readonly" />
						</td>
						<td>'.$hasil['satuan_kpr'].'</td>
						<td><input type="text" name="diskon[]" id="pdiskon'.$nomor.'" class="inputangka" value="'.$hasil['diskon_tsd'].'" onchange="hitungsalesretur('.$nomor.')" placeholder="0" readonly="readonly" /></td>
