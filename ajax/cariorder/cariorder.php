<?php
	...
	while($hasil= $master->fetch(PDO::FETCH_ASSOC)){
		// Query Cek harga program jika program dipilih
		$harga_pakai = $hasil['harga_phg'];
		$badge_program = '';
		if($id_program > 0){
			$cek_prog = $conn->prepare("SELECT B.harga_program FROM program_produk AS A LEFT JOIN program_produk_detail AS B ON A.id_pp=B.id_pp WHERE A.id_pp=:id_program AND B.id_pro=:id_pro");
			$cek_prog->bindParam(':id_program', $id_program, PDO::PARAM_INT);
			$cek_prog->bindParam(':id_pro', $hasil['id_pro'], PDO::PARAM_STR);
			$cek_prog->execute();
			$prog_row = $cek_prog->fetch(PDO::FETCH_ASSOC);
			if($prog_row){
				$harga_pakai = $prog_row['harga_program'];
				$badge_program = ' <span class="badge badge-success" title="Harga Program"><i class="fas fa-tag"></i></span>';
			}
		}
		$subtot	= $harga_pakai * $hasil['jumlah_tod'];
		...
		$tabel	.= '<tr id="traddorder'.$nomor.'">
					<td>
					<a onclick="addrorder('.$nomor.', '.$hasil['id_tod'].')"><i class="fa fa-plus-circle"></i></a> '.$hasil['nama_pro'].$badge_program.'
					...
					<td><input type="text" name="harga[]" id="pharga'.$nomor.'" class="inputangka" onkeyup="angka(this)" value="'.$data->angka($harga_pakai).'" placeholder="0" readonly="readonly" /></td>
	...
	$conn	= $base->close();
	$json	= array("tabel" => $tabel, "supplier" => $view['nama_sup'], "tglorder" => $view['tgl_tor'], "ketorder" => $view['ket_tor'], "jatuhtempo" => $limit, "jumaddorder" => $view['jitem'], "footer" => $footer, "id_program" => $id_program);
