<?php
	...
	$paging	= new Paging;
	$conn	= $base->open();
	$jumlah	= $conn->query("SELECT COUNT(*) AS jumlah FROM transaksi_faktur AS A LEFT JOIN outlet AS C ON A.id_out=C.id_out WHERE A.status_tfkkf='Belum Dikirim'")->fetch(PDO::FETCH_ASSOC);
	if(empty($jumlah['jumlah'])){
		...
		$master	= $conn->prepare("SELECT A.kode_tfk, A.tgl_tfk, A.total_tfk, A.tgl_limit,A.status_tfkkf,C.nama_out FROM transaksi_faktur AS A LEFT JOIN outlet AS C ON A.id_out=C.id_out WHERE A.status_tfkkf='Belum Dikirim' ORDER BY A.created_at ASC");
