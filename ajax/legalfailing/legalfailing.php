...
<tr id="<?php echo("no_faktur$nomor"); ?>">
	...
	<select name="no_faktur[]"  id="no_faktur<?php echo($nomor); ?>" class="form-control select2" required="required">
		...
		<?php
                 ...
                 $master = $conn->prepare("
                                            SELECT A.id_tfk, A.tgl_tfk, A.kode_tfk, A.total_tfk, B.nama_out, A.status_failing, 'Cendo' AS sumber
                                            FROM transaksi_faktur AS A
                                            INNER JOIN outlet AS B ON A.id_out = B.id_out
                                            WHERE A.status_failing = :status AND YEAR(A.tgl_tfk) IN (2025, 2026) 
                                        
                                            UNION ALL
                                        
                                            SELECT A.id_tfk, A.tgl_tfk, A.kode_tfk, A.total_tfk, B.nama_out, A.status_failing, 'PIM' AS sumber
