<?php
	...
	if($id_program > 0){
		...
		$cek_prog->bindParam(':id_pro', $view['id_pro'], PDO::PARAM_STR);
		$cek_prog->execute();
	...
	$subtot	= $harga_pakai * 1;
	$total	= $subtot - (($subtot * 0) / 100);
	//$jumlah	= $_GET['jumlah'];
	$nomor	= ($jumlah + 1);
	$tabel	= '<tr id="traddorder'.$nomor.'">
					<td>'.$view['nama_pro'].$badge_program.'<input type="hidden" name="product[]" value="'.$view['id_pro'].'" readonly="readonly" /></td>
