...
$q = "
    SELECT
           ...
           O.nama_out,
           A.created_at AS tanggal_failing,
           A.status_failing
...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    $sheet->setCellValue('D' . $rowNum, $row['tanggal_failing'] ?? '');
    $sheet->setCellValue('E' . $rowNum, $row['status_failing'] ?? '');
    $rowNum++;
}
...
$filename = 'report_dokumen_failing_' . date('Ymd_His') . '.xlsx';
