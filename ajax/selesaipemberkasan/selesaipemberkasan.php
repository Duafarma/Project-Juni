...
<tr id="<?php echo("no_faktur$nomor"); ?>">
    <td>
	<!-- gunakan id unik supaya select2 bisa di-init ulang dan menampilkan search -->
	<select name="no_faktur[]" id="no_faktur_<?php echo $nomor; ?>" class="form-control select2" required="required">
        ...
        <?php
                //  $status	= 'sudah failing';
                 $master = $conn->prepare("SELECT A.id_tfk,A.tgl_tfk, A.status_tfk, A.kode_tfk,A.total_tfk,B.nama_out,A.status_dokumentasi FROM transaksi_faktur AS A INNER JOIN outlet AS B ON A.id_out=B.id_out 
                                           LEFT JOIN serah_terima_faktur_pemberkasan_detail AS C ON A.id_tfk=C.no_faktur 
                                           WHERE 
                                                A.tgl_tfk >= DATE_SUB(NOW(), INTERVAL 100 DAY) AND
                                                C.no_faktur IS NULL AND(A.status_dokumentasi='sudah siap' OR
                                                A.status_tfk IN ('Faktur'))   ORDER BY A.tgl_tfk DESC");
...
?>
<!-- inisialisasi hanya select yang baru ditambahkan supaya muncul kotak search -->
<script type="text/javascript">
    setTimeout(function(){
        ...
        sel.select2({
            ...
            width: '100%',
            allowClear: true,
            minimumResultsForSearch: 0 // paksa tampilkan kotak pencarian
        });
    }, 50);
</script>
